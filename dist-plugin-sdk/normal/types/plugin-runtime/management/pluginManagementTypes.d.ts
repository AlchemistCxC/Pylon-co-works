import type { PluginContractDiagnostic } from '../pluginContractResolver.js';
import type { PluginDeactivateResult } from '../pluginInstance.js';
import type { InstalledPluginPackage } from '../../infrastructure/plugins/pluginPackageClient.js';
/**
 ...
 */
export declare const PLUGIN_MANAGEMENT_CAPABILITY: "plugin.management";
export type PluginManagementErrorCode = 'management_self_locked' | 'management_product_required' | 'management_not_authorized';
export declare class PluginManagementError extends Error {
    readonly code: PluginManagementErrorCode;
    readonly pluginId: string;
    constructor(code: PluginManagementErrorCode, pluginId: string, message: string);
}
...
export interface PluginManagementDeps {
    ...
    installOrUpdate(sourcePath: string): Promise<{
        ok: boolean;
        message?: string;
    }>;
    /** 内置侧启用/停用（→ runtime.disable / retryBuiltinPlugin）。 */
    setBuiltinEnabled(pluginId: string, enabled: boolean): Promise<{
        ok: boolean;
        message?: string;
    }>;
}
