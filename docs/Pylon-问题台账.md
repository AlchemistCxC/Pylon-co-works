...
追加修正（2026-09-01）：空态 Composer 宽度提升至最大 `900px`（外层承载 `960px`），输入区最小高度提升至 `144px`，上下文/选项/底部操作栏形成连续表面；terminal-like 保留方形边界但不再透明到难以识别。创建、附件和失败恢复逻辑未改动。`npm.cmd run check:solid` 通过。
