...
| B-01 | `src/index.css`；`src/themeFieldDefs.ts`；`src/domains/theme/visualSemantics.ts`；`src/domains/theme/themeCssSnapshot.ts`；新增 `src/domains/theme/__tests__/B-01-visual-semantic-palette.test.ts` | App/store/customPresets/preset/runtime/backend、高风险接口、线路 A/C、其他产品文件均未改；公共角色仅扩展现有 visualSemantics，未新增 resolver/registry；既有外部改动未触碰 | 相关主题/skin/profile targeted Vitest：9 files / 57 tests passed；targeted ESLint（TS/测试）exit 0；`npm.cmd run check:first-party-styles`（28 files）exit 0；`git diff --check` exit 0。覆盖四套 fallback 角色投影、显式 preset 候选和 scheme probe 回退，不断言固定像素 | 发布 12 个角色 token、三角色 type/disabled/radius alias；复用 `visualSemantics → themeFieldDefs → themeCssSnapshot` 单链，兼容旧变量 alias | `61a49cdd`；已提交，等待根 agent 复核；局部 rgba/背景图合成仍需运行态 contrast QA |
| B-02 | `src/components/chat/InputBar.tsx`；`src/components/chat/ChatView.tsx`；`src/renderers/solid-workbench/chat/ToolCard.solid.tsx`；`src/renderers/solid-workbench/chat/ToolInvocationCard.solid.tsx`；新增 `src/components/chat/__tests__/B-02-input-suggestion.test.ts`、`src/renderers/solid-workbench/chat/__tests__/B-02-state-aria.solid.test.tsx` | store/preset/runtime/backend、领域状态机、其他线路 CSS/TSX、总契约/原始施工书均未改；保留建议数据/过滤/Enter/Arrow/click 逻辑；未重复 B-18 | `npm.cmd run test:frontend -- src/components/chat/__tests__/B-02-input-suggestion.test.ts src/renderers/solid-workbench/chat/__tests__/B-02-state-aria.solid.test.tsx`：2 files / 8 tests passed；targeted ESLint（4 个产品 TSX + 2 个 B 测试）exit 0；`npm.cmd run check:solid` exit 0（边界/skin/product/renderer checks passed）；`git diff --check` exit 0 | DF-01：suggestion 改为 `button type="button"`，保留 `cmd-item` class；工具卡补可读状态 label、`data-status-label`；空消息会话补 status 空态 DOM | 待提交；提交后状态 READY_FOR_REVIEW，等待根 agent 复核；A/C 需联验 CSS/窄视口与运行态空态 |
| B-03 | `src/components/Settings.tsx`；新增 `src/components/__tests__/B-03-settings-dialog.test.tsx` | store/preset/runtime/backend、设置字段/保存/删除/导航业务、线路 A/C CSS、总契约/原始施工书均未改；窗口系统按钮仍由宿主外层管理 | `npm.cmd run test:frontend -- src/components/__tests__/B-03-settings-dialog.test.tsx src/components/__tests__/Settings.agentOnboarding.test.tsx src/components/settings/__tests__/SettingsSectionHeader.test.tsx`：3 files / 10 tests passed；targeted ESLint exit 0；`git diff --check` exit 0。覆盖 dialog/aria-labelledby、打开聚焦、Tab/Shift+Tab 循环和关闭焦点恢复 | `role="dialog"` + `aria-modal="true"` + 标题关联；新增本地焦点栈，不改字段与导航契约 | 待提交；提交后状态 READY_FOR_REVIEW，等待根 agent 复核；真实 Tauri 标题栏三按钮例外需跨线验证 |
| B-04 | `src/index.css`（语义 fallback/disabled alias 已含于 B-01）；新增 `src/domains/theme/__tests__/B-04-visual-conformance.test.ts` | 未改 store/preset/runtime/backend、线路 A/C、总契约/原始施工书；插件内部几何不纳入断言；dev server 仅用于只读 computed QA，已停止 | `npm.cmd run test:frontend -- src/domains/theme/__tests__/B-04-visual-conformance.test.ts`：1 file / 5 tests passed；targeted ESLint exit 0；`npm.cmd run check:first-party-styles`（28 files）exit 0；浏览器 :5174 四组合 computed 记录见下方证据；`git diff --check` exit 0 | DF-03a/DF-03p 角色安全回退 QA；DF-06/DF-08/DF-09 focus/reduced-motion 规则静态守卫；不新增角色/registry | 待提交；提交后状态 READY_FOR_REVIEW，等待根 agent 复核；浏览器字体资源在 junction node_modules 下有 Vite allow-list 警告，不影响 CSS role/border computed 读取 |
...
**B-04 浏览器 computed 证据（隔离 dev :5174，刷新后读取）**

| mode/scheme | `--surface-canvas` / `--content-text` | `--stroke-default` / `--connector-default` | 实际边界采样 |
|---|---|---|---|
| terminal-like/light | `#F3F1EC` / `#1C211E` | `#7A847E` / `#69746D` | `.term-tool` border `rgb(122,132,126)`；CLI input top/bottom `rgb(105,116,109)` |
| terminal-like/dark | `#141916` / `#F1F4F2` | `#718178` / `#718178` | `.term-tool` border `rgb(113,129,120)`；CLI input top `rgb(113,129,120)` |
| modern-gui/light | `#F5F7FC` / `#1B2030` | `#7986A1` / `#6B7892` | `.term-tool` border `rgb(121,134,161)` |
| modern-gui/dark | `#181C27` / `#F7F8FC` | `#6F7A95` / `#6F7A95` | `.term-tool` border `rgb(111,122,149)` |

静态对比度守卫对四套 fallback 逐角色计算：`content.text`/`content.muted` ≥ `4.5:1`，`stroke`/`accent`/`state.*`/`focusRing`/`connector` ≥ `3:1`；仅记录比值门槛，不锁定像素快照。
