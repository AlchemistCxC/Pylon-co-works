...
将设置页的导航、深链和搜索入口统一到 `settingsDomains` 注册表；每个设置项保持一个清晰的域/分区归属，旧入口可继续工作但不再产生“幽灵分区”。Slice A（设置意图归一化）、Slice B（字段标签审计）与 Slice C（贡献注册表搜索投影）已落地，后续再处理失效入口清理。
...
审计发现“背景图/模糊/背景不透明度/背景色”是不同 section 的不同字段，并非重复注册；但在全局快速搜索和无上下文读屏时容易混淆。已将作用域写入 canonical label（如“侧栏背景图”“消息流模糊”“右栏背景不透明度”），不改变持久化 key、默认值或 CSS 变量。

`themeFieldCopy.test.ts` 新增可见字段标签唯一性守卫；未来新增同名字段必须显式加入作用域或调整术语。搜索仍可通过 canonical label 的中文子串命中旧的通用词。

## Slice C：贡献注册表搜索投影

插件设置页本身是 opaque component，宿主没有字段 schema；因此速搜索引收录“插件设置页”级条目，携带 `pluginPageId`，选择后直接打开对应贡献页。缺失/已停用贡献仍由 `PluginSettingsPageHost` 显示不可用提示，不伪造字段锚点。
