...
Plugin API 1.0 不使用旧版的 `trust`、`capabilities`、`signature` 或 Ed25519 白名单字段。
...
安装成功后，插件会立即激活，并默认保持启用。下次启动时 Pylon 会恢复已启用插件。
...
激活已安装插件，并把启用状态持久化。若激活失败，Pylon 会恢复为停用状态。
...
停用插件并删除安装包。默认保留插件数据；当前设置页的普通“卸载”不会执行 purge data。
...
2. 入口模块激活成功后再启用样式；
3. 激活失败时删除候选样式；
4. 停用、重载或更新时随 PluginScope 删除旧样式。

Pylon 当前不提供 CSS selector 沙箱。插件作者应使用插件 id 前缀或稳定 `data-*` 属性限制选择器作用域。

Pylon 1.3.0 有两个顶层界面模式：Modern GUI 与 Terminal-like。插件样式可以使用 `[data-interface-mode="modern-gui"]` 或 `[data-interface-mode="terminal-like"]` 只影响指定模式。切换界面模式不会停用插件，也不会切换六套主题预设。
...
打开“设置 → 插件 → 最近日志”。单个外置插件失败不会阻止 Pylon 和其他插件启动。
...
第一方 `builtin.pylon-shell` 被停用后，Pylon 会显示 Kernel Recovery 界面。Kernel 仍在运行，可重新启用或重新挂载 Shell。

---

## 12. 当前边界

- 当前稳定契约是 Plugin API 1.0；旧 API 0.1 已删除，不兼容旧 manifest。
- 当前没有插件市场、签名商店或权限审批 UI。
