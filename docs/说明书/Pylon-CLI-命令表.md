...
| `interaction list` | — | 全部挂起权限请求快照（含应答所需 identity、应答词项 `kind` 与 options） |
| `interaction respond <requestId> <optionId>` | — | 应答权限请求（optionId 经合法性校验；`kind` 取列表条目透传，权限请求恒为 `approval`，#36） |
