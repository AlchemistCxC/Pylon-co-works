...
六个第一方 Product Plugin 标记为“产品运行必需”，运行中不能通过普通设置操作停用。若启动阶段某个必需插件失败，Pylon 会先显示 Kernel Recovery 界面，并提供定向重试或“进入安全模式”。安全模式不会自动启动 Product Plugin 和用户插件；可以逐个尝试启动，也可以选择“正常启动”。
