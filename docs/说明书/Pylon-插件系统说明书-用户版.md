...
>
> 插件契约：Plugin API 1.0–1.3 与 2.0–2.4（2.0 为破坏性升版：左栏贡献改为 region 堆叠模型），`pylon-plugin.json` schema 1
...
同排亦有“从 zip 安装…”“从 URL 安装…”入口，见 §12。
...
| 启动失败（等待能力授权） | 插件声明了宿主能力（API 1.2）但未获批：行内徽标是"启动失败"，"启动故障"区出现注明"等待能力授权"的可重试条目，页顶授权卡可批准（见 §5.1） |
...
即插件管理面）。声明了能力但尚未获得你批准的插件不会被激活——插件列表徽标是
"启动失败"，"启动故障"区会出现注明"等待能力授权"的可重试 `capability-consent` 条目；外置包安装后
...
- 保证强度说明：插件在本机以你的用户身份运行（可信本机代码边界），能力授权模型约束的是"诚实的插件做声明过的事"，不是恶意代码沙箱——请只安装来源可信的插件。

已授权插件的管理面板会出现在插件设置页：内置"插件管理器"由
「设置 → 插件 → 插件管理」分区直接承载（不再单列独立条目），
其余插件贡献的设置页则各自单列。未授权时面板只显示授权引导，
不会报错。声明了 `plugin.management` 的插件**不能管理它自己**（防自拆），
也不能停用产品运行必需的内置组件。
...
Pylon 内置三个顶层界面模式：现代 GUI（modern-gui）、经典终端（terminal-like）与 蓝调战术（tactical-blue）；插件也可以通过 `interfaceModes.registerMode` 注册自己的完整模式（含 workbench 声明，见开发者版）。插件样式可以使用 `[data-interface-mode="…"]` 只影响指定模式。切换界面模式不会停用插件，也不会切换内置主题预设（当前 10 套）。
...
- `api` 是否为宿主支持的版本（`1.0`–`1.3`、`2.0`–`2.4`）；
- `schema` 是否为 `1`；
- `web.entry` 指向的文件是否存在；
...
七个第一方 Product Plugin 标记为“产品运行必需”，运行中不能通过普通设置操作停用。若启动阶段某个必需插件失败，Pylon 会先显示 Kernel Recovery 界面，并提供定向重试或“进入安全模式”。安全模式不会自动启动 Product Plugin 和用户插件；可以逐个尝试启动，也可以选择“正常启动”。
...
- 当前稳定契约是 Plugin API 1.0–1.3 与 2.0–2.4（1.1 新增插件私有存储；1.2 新增能力声明与用户授权，见 §5.1；**2.0 为破坏性升版**——Agent 左栏贡献从按 `mode` 的互斥视图改为按 `region` 的堆叠区块；2.1–2.4 依次新增标题栏 app-menu、右栏亲和模型、会话置顶、命令 keywords/tier）；旧 API 0.1 已删除，不兼容旧 manifest。
