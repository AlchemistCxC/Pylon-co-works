...
| markdown 与高亮（WASM） | `src-tauri/pylon-markdown/src/` | comrak（GFM）解析 → 与 TS `MarkdownRenderNode` 同形状的渲染模型；syntect + **vendored tmLanguage 语法**（`assets/`，由 `gen/generate-assets.mjs` 从 starry-night 导出）做**整块进/整块（行数组）出**的高亮。不含 DOM 与缓存编排（那些留 JS：`chat/markdownRenderModel.ts` 解析缓存/graft、`chat/codeBlockDomLifecycle.ts` 高亮 DOM 生命周期——视口门控、圈外降级+行缓存、帧预算调度，#221） | `cargo test -p pylon-markdown --lib`；`parity/` 下的差分工具与快照（`diff.mjs` / `parity-report.json`） |
