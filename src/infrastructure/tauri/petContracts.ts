...
/**
 * ISSUE-18 W1（LR2-WI08）：Pet 状态权威源信封——统一 local/backend 装载、仲裁与写回。
 * 解决「前端 localStorage 与后端 pylon-pet.json 双源，启动时 localStorage 无条件 restore
 * 覆盖后端」（ISSUE-18 问题 1）。仲裁键 = 状态突变时间戳（last_tick_at_ms）。
 */
export interface PetStateEnvelope {
  state: PetState
  /** 状态突变时间（毫秒）；仲裁键，来源为 PetState.last_tick_at_ms */
  updatedAtMs: number
  source: 'backend' | 'local'
}

export function toPetStateEnvelope(state: PetState, source: 'backend' | 'local'): PetStateEnvelope {
  return { state, updatedAtMs: state.last_tick_at_ms, source }
}

/**
 * 权威源仲裁：较新（updatedAtMs 更大）者胜；相等/缺失时 backend 优先（后端为单一
 * 权威源）。**禁止无条件 localStorage 覆盖**（旧 restore 行为即该 bug）。
 */
export function resolvePetStateConflict(
  local: PetStateEnvelope | null,
  backend: PetStateEnvelope | null,
): PetStateEnvelope | null {
  if (!local) return backend
  if (!backend) return local
  return local.updatedAtMs > backend.updatedAtMs ? local : backend
}
