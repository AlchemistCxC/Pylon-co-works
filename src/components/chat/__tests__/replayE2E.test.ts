...
function makeSession(source: string): Session {
  return {
    id: `sess-${source}`, source, name: 'session-x', profileId: 'p1', createdAt: 1,
...
describe('回放去重端到端（验证 agent 对抗场景）', () => {
  it('live tool 消息（不同 toolCallId）不被 replay 权威误删', async () => {
    ...
    const userListener = listeners.get('pylon:user')
    expect(userListener).toBeDefined()
    handle.commitReplaySnapshot(source, [
  ...
  it('live 乐观 user 消息 + replay 权威含同内容 → 不重复', async () => {
    ...
    // A 只能出现一次；old-q 一次；old-a 一次
    expect(contents.filter(c => c === 'A').length).toBe(1)
    expect(contents.filter(c => c === 'old-q').length).toBe(1)
    expect(contents.filter(c => c === 'old-a').length).toBe(1)
    // 顺序：replay 权威在前，live 增量（去重后）在后
  ...
  it('snapshot 提交必须整体替换旧 runtime，不能把上轮残留重新合并', async () => {
    ...
    const resolved = handle.commitReplaySnapshot(source, [
  ...
  it('空 replay snapshot 清空旧缓存，不能保留另一个会话的显示', async () => {
    ...
    expect(handle.commitReplaySnapshot(source, [], cached)).toEqual([])
