...
import {
  ...
  type PetGrowthStage,
  type PetState,
  type PetStateEnvelope,
  type PetStats,
} from '../infrastructure/tauri/petContracts'
...
export default function PetCompanion({ rightInset = 0 }: { rightInset?: number }) {
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        ...
        if (savedRaw) {
          try {
            localEnvelope = toPetStateEnvelope(normalizePetState(JSON.parse(savedRaw)), 'local')
          } catch {
            // 损坏本地缓存：忽略，走后端权威
          }
        }
        const winner = resolvePetStateConflict(
          localEnvelope,
          toPetStateEnvelope(backendState, 'backend'),
        )
        if (!winner) return
  ...
  // I18 W3（LR2-WI10）：nostalgia = pet_action 回忆（recall_memory → msg 显示回忆）
  const runAction = (action: 'feed' | 'play' | 'equip' | 'unequip' | 'nostalgia', value?: string) => {
    if (!IS_TAURI) {
  ...
  return (
    <section ref={shellRef} className={`pet-companion ${dragging ? 'dragging' : ''} ${poking ? 'poking' : ''} ${perched ? 'perched' : ''} ${tabletCoding ? 'tablet-coding' : ''} behavior-${behavior} pose-${pose}`}
      ...
      {panelOpen && <div className="pet-panel" onPointerDown={event => event.stopPropagation()}>
        ...
        {/* I18 W3（LR2-WI10）：回忆与成就——nostalgia 可达入口 + 成就徽章 + 长期记忆展示 */}
        <div className="pet-memories" aria-label="宠物回忆与成就">
          <div className="pet-panel-label">回忆与成就</div>
          <div className="pet-care-controls">
            <button type="button" onClick={() => runAction('nostalgia')}>回忆</button>
          </div>
          <div className="pet-cosmetic-list">
            {pet.achievements.filter(item => item.unlocked).map(item => <span key={item.id} className="pet-cosmetic-swatch swatch-glow" title={`${item.name}：${item.description}`}>{item.name}</span>)}
            {pet.achievements.filter(item => item.unlocked).length === 0 && <span className="pet-panel-empty">还没有解锁成就</span>}
          </div>
          <div className="pet-cosmetic-list">
            {[...pet.memories].slice(-3).reverse().map((memory, index) => <span key={index} className="pet-cosmetic-swatch swatch-hat" title={memory}>{memory}</span>)}
            {pet.memories.length === 0 && <span className="pet-panel-empty">还没有形成长期记忆</span>}
          </div>
        </div>
      </div>}
