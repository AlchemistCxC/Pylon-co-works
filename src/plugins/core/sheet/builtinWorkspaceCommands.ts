...
export function createBuiltinWorkspaceCommandDefinitions(): CommandDefinition[] {
  const base = 400
  return [
    { id: 'layout.inspect', name: 'layout.inspect', description: '读取共享布局状态', priority: base, execute: () => { const s = useWorkspaceStore.getState(); return { sidebarWidth: s.sidebarWidth, sidebarCollapsed: s.sidebarCollapsed, rightPanelCollapsed: s.rightPanelCollapsed, showPet: s.showPet } } },
    { id: 'layout.sidebar.set', name: 'layout.sidebar.set', description: '设置所有 Sheet 共享的左栏折叠状态', priority: base + 1, execute: ({ args }) => { const value = boolean(record(args).collapsed, 'collapsed'); useWorkspaceStore.getState().setSidebarCollapsed(value); return { collapsed: value } } },
    { id: 'layout.sidebar-width.set', name: 'layout.sidebar-width.set', description: '设置共享左栏宽度', priority: base + 2, execute: ({ args }) => { const width = record(args).width; if (typeof width !== 'number' || !Number.isFinite(width)) throw new Error('width 必须是数字'); useWorkspaceStore.getState().setSidebarWidth(width); return { width: useWorkspaceStore.getState().sidebarWidth } } },
    { id: 'layout.right-panel.set', name: 'layout.right-panel.set', description: '设置共享右栏折叠状态', priority: base + 3, execute: ({ args }) => { const value = boolean(record(args).collapsed, 'collapsed'); useWorkspaceStore.getState().setRightPanelCollapsed(value); return { collapsed: value } } },
