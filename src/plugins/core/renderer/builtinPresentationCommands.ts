...
export function createBuiltinPresentationCommandDefinitions(): CommandDefinition[] {
  ...
  return [
    ...
    { id: 'presentation.apply', name: 'presentation.apply', description: '应用呈现风格，不重置背景和用户资产', priority: base + 2, execute: ({ args }) => { const profileId = id(record(args).profileId, 'profileId'); const entry = getPresentationProfileRegistry().resolve(profileId); if (!entry) throw new Error(`呈现风格不存在：${profileId}`); applyPresentationProfile(entry.value, { setZoneField: (zone, patch, source) => useStore.getState().setZoneField(zone, patch, source), setActiveProfileId: next => usePresentationPreferenceStore.getState().setActiveProfileId(next) }); return { profileId } } },
  ]
}
