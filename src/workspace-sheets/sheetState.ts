...
export function sheetReducer(state: SheetState, action: SheetAction): SheetState {
  switch (action.type) {
    case 'open': {
      ...
      try {
        workspaceState = registry.serialize(registry.createInitialState(action.sheet.state ?? action.sheet.metadata))
      } catch {
        return withActiveFallback(state)
      }
      ...
      const sheet: SheetRecord = {
        ...
        ...(workspaceState !== undefined ? { state: workspaceState } : {}),
      }
