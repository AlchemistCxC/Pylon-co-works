...
export function applyGlobalPreset(name: PresetName | string, ports?: GlobalPresetTransactionPorts): boolean {
  ...
  if (!preset.presentationProfileId) {
    ;(ports?.theme ?? useStore).getState().setGlobalPreset(preset.name, preset.theme)
    return true
  }
  const profile = getPresentationProfileRegistry().resolve(preset.presentationProfileId)?.value
  if (!profile || !activatePresentationProfile(profile.id, ports)) return false
  ;(ports?.theme ?? useStore).getState().setGlobalPreset(preset.name, { ...preset.theme, ...profile.tokens })
  return true
}
