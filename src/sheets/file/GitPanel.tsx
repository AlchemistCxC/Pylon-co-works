...
function GitStatusTree({ entries }: { entries: GitStatusEntry[] }) {
  ...
  const render = (items: GitPathNode[], depth: number): React.ReactNode => items.map(node => {
    ...
    return (
      <li key={node.key} className="git-tree-node">
        <button type="button" className="git-tree-row" style={{ '--git-tree-depth': Math.min(depth, 7) } as React.CSSProperties} onClick={() => folder && toggle(node.key)} title={node.path || node.key}>
...
export default function GitPanel({ source }: { source: string | null }) {
    ...
    <div className="file-section-panel git-panel">
      ...
      <section className="git-section">
        ...
        <ul className="git-history-list">
          {history.map(commit => (
            <li key={commit.hash} className="git-history-row">
