...
export default function WorkspaceSearchPanel({ target, provider, onOpenResult }: {
  ...
  const search = async () => {
    if (!target || !targetKey || !provider?.search || !query.trim()) return
    const token = beginSourceRequest(requestContext.current, targetKey)
    const requestId = ++searchRequestId.current
    setStatus({ kind: 'searching' })
    try {
      const raw = await provider.search(target, query.trim())
    ...
    <div className="file-section-panel file-search-panel">
      <div className="file-search-toolbar">
        <input
          ...
          aria-label="工作区搜索"
        />
        <button type="button" className="file-search-submit" onClick={() => void search()} disabled={status.kind === 'searching' || !target || !provider?.search || !query.trim()} aria-label="搜索">
          <Search size={15} />
        </button>
      </div>
