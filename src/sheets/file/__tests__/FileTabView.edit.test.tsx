...
describe('FileTabView 编辑器模式（0-A1 单内核）', () => {
  ...
  it('editing=true → 内核可编辑，输入使摘要 dirty 翻转（不改文件）', async () => {
    const { summaries } = renderEditor({ editing: true })
  ...
  it('editing=false → 内核只读档；rerender 翻转 editable（同一实例不重挂）', async () => {
    const { onSummaryChange, apiRef, rerender } = renderEditor({ editing: false })
    await waitForFileEditor('const x = 1')
    expect(fileEditorEditable()).toBe(false)

    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={vi.fn()} editing onSummaryChange={onSummaryChange} apiRef={apiRef} />)
  ...
  it('编辑中 touchVersion 递增且磁盘 ≠ 编辑内容 → onExternalChange 上报，编辑内容不被覆盖', async () => {
    ...
    const { apiRef, onSummaryChange, rerender, summaries } = renderEditor({ editing: true, onExternalChange })
    const editor = await waitForFileEditor('const x = 1')
    // 用户编辑
    ...
    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={vi.fn()} editing onExternalChange={onExternalChange} onSummaryChange={onSummaryChange} apiRef={apiRef} />)
  ...
  it('编辑中 touchVersion 递增但用户未改 → 内容安全刷新到磁盘', async () => {
    ...
    const { onSummaryChange, apiRef, rerender } = renderEditor({ editing: true, onExternalChange, onContentReady })
    await waitForFileEditor('const x = 1')
    invoke.mockImplementation((cmd: string) => {
      if (cmd === 'read_workspace_text') return Promise.resolve(readTextResult('const x = 2'))
      return Promise.reject(new Error(`unexpected invoke ${cmd}`))
    ...
    useWorkspaceStore.setState({ touchVersions: { [touchedFileVersionKey({ agentId: 'agent-test', source: 'ws-a' }, 'src/a.ts')]: 7 } })
    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={onContentReady} editing onExternalChange={onExternalChange}  onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    await waitForFileEditor('const x = 2')
    expect(onExternalChange).not.toHaveBeenCalled()
    expect(onContentReady).toHaveBeenCalledWith('const x = 2')
  })

  it('只读模式 touchVersion 递增 → 重拉语义保留（文档替换 + 变更行 decoration + onContentReady）', async () => {
    const onContentReady = vi.fn()
    const { onSummaryChange, apiRef, rerender } = renderEditor({ editing: false, onContentReady })
    await waitForFileEditor('const x = 1')
    invoke.mockImplementation((cmd: string) => {
      if (cmd === 'read_workspace_text') return Promise.resolve(readTextResult('const x = 2'))
      return Promise.reject(new Error(`unexpected invoke ${cmd}`))
    ...
    useWorkspaceStore.setState({ touchVersions: { [touchedFileVersionKey({ agentId: 'agent-test', source: 'ws-a' }, 'src/a.ts')]: 7 } })
    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={onContentReady} onSummaryChange={vi.fn()}  onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    await waitForFileEditor('const x = 2')
    expect(onContentReady).toHaveBeenCalledWith('const x = 2')
  ...
  it('退出编辑模式不静默覆盖未保存修改：touchVersion 已定义时退出编辑，磁盘内容不得替换未保存编辑', async () => {
    const onExternalChange = vi.fn()
    const { onSummaryChange, apiRef, rerender } = renderEditor({ editing: true, onExternalChange })
    const editor = await waitForFileEditor('const x = 1')
    replaceFileEditorValue(editor, 'const x = 999')
  ...
  it('带未保存内容重进编辑模式不误报冲突：touchVersion 已定义且磁盘未变，probeDisk 不触发 onExternalChange', async () => {
    ...
    rerender(<FileTabView source="ws-a" path="src/a.ts" onTruncated={vi.fn()} onContentReady={vi.fn()} editing onExternalChange={onExternalChange} onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    await act(async () => { await vi.advanceTimersByTimeAsync(400) })
    expect(onExternalChange).not.toHaveBeenCalled()
    expect(fileEditorView().state.doc.toString()).toBe('const x = 999')
    vi.useRealTimers()
  ...
  it('saveAnchorToken 递增 → 重拉磁盘对齐（保存后的磁盘锚点推进）', async () => {
    const onContentReady = vi.fn()
    const { onSummaryChange, apiRef, rerender } = renderEditor({ editing: true, onContentReady })
    await waitForFileEditor('const x = 1')
    invoke.mockImplementation((cmd: string) => {
      if (cmd === 'read_workspace_text') return Promise.resolve(readTextResult('const x = 3'))
      return Promise.reject(new Error(`unexpected invoke ${cmd}`))
    })
    rerender(<FileTabView source="ws-a" path="src/a.ts" onTruncated={vi.fn()} onContentReady={onContentReady} editing saveAnchorToken={1} onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    await waitForFileEditor('const x = 3')
  })

  it('保存回执：无更新编辑 → 锚点推进 + 变更标记清空；有更新编辑 → 保留编辑只推进锚点', async () => {
    const { apiRef, onSummaryChange, rerender, summaries } = renderEditor({ editing: true })
    const editor = await waitForFileEditor('const x = 1')
    replaceFileEditorValue(editor, 'const x = 2')
    expect(lastSummary(summaries).dirty).toBe(true)

    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={vi.fn()} editing saveReceipt={{ version: 1, expectedContent: 'const x = 2', persistedContent: 'const x = 2' }}  onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    ...
    rerender(<FileTabView source="ws-a" path="src/a.ts" context={{ agentId: 'agent-test', source: 'ws-a' }} onTruncated={vi.fn()} onContentReady={vi.fn()} editing saveReceipt={{ version: 2, expectedContent: 'const x = 2', persistedContent: 'const x = 2' }}  onSummaryChange={onSummaryChange} apiRef={apiRef} />)
    await act(async () => {})
