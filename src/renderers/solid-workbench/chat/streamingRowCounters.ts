 ...
 *
 * 为什么需要它：行集合必须始终是当前文本的函数。`rows / paragraphs > 1` 说明行集合里
 * 出现了文本里并不存在的边界——历史上「每几个字换行」的碎裂（issue #55）就是它的后果；
 * `retreats` 说明发布链出现过非后继输入（插值前缀、双列表分叉、终态重发、resume 之一）。
 * 两者都是本 issue 的现场判据，且不依赖任何 DOM 读取。
 *
 * 计数是模块级累计量：读数是「本次运行内所有流式行」的聚合，不持有任何引用，因此不会
 * 影响行组件的回收；`lastRows`/`lastParagraphs` 表示最近一次发布（多行并发时为最后发布者）。
 */
export interface StreamingRowCountersSnapshot {
  /** 记账过的发布次数 */
  readonly publications: number
  /** 输入不是上一次文本后继的次数（回退/换挡/重放） */
  readonly retreats: number
  /** 最近一次发布的行数与文本段落数 */
  readonly lastRows: number
  readonly lastParagraphs: number
  /** 最近一次的 rows / paragraphs（段落数为 0 时为 0） */
  readonly rowsPerTextLength: number
  /** 历史最大 rows / paragraphs；**> 1 即行集合出现文本之外的边界** */
