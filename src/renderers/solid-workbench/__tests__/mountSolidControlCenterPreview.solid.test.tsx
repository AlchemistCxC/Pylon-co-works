...
describe('mountSolidControlCenterPreview', () => {
  ...
  it('注册发送控件后按位置渲染底层块，hidden 时不渲染并注入几何变量', async () => {
    ...
    expect(theme.sendButtonIconColor).toBe('white')
    ...
    try {
      ...
      expect(controlCenter?.style.getPropertyValue('--cc-send-icon-color')).toBe('#fff')
      ...
      expect(controlCenter?.style.getPropertyValue('--cc-send-icon-color')).toBe('#fff')
      services.appearance.setTheme({ ...theme, sendButtonIconColor: 'gray', inputSubmitButtonMode: 'external' })
      await waitFor(() => expect(controlCenter?.style.getPropertyValue('--cc-send-icon-color')).toBe('rgba(0,0,0,.5)'))
      services.appearance.setTheme({ ...theme, sendButtonIconColor: 'black', inputSubmitButtonMode: 'external' })
      await waitFor(() => expect(controlCenter?.style.getPropertyValue('--cc-send-icon-color')).toBe('#000'))
