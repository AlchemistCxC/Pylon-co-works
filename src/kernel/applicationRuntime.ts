...
export function createApplicationRuntime(): ApplicationRuntime {
  ...
  return {
    registerBuiltin(contribution) {
      return register({
        pluginId: 'kernel.application-test',
        version: 'builtin',
        packageInstanceId: 'kernel.application-test@builtin',
        runtimeInstanceId: `kernel.application-test@${contribution.id}`,
    ...
    beginShadowTransaction(owner, replacingRuntimeInstanceId) {
      ...
      const candidate = () => {
        ...
        for (const [id, entry] of next) {
          if (entry.ownerRuntimeInstanceId === replacingRuntimeInstanceId) next.delete(id)
        }
      ...
      return {
        register: contribution => {
          ...
          item.proxy = {
            dispose: () => {
              ...
              publish(snapshot.activeApplicationId === id ? null : snapshot.activeApplicationId)
            },
          }
          staged.push(item)
          return item.proxy
        ...
        commit: () => {
          ...
          settled = true
          committed = true
          previousEntries = new Map(contributions)
          previousActiveApplicationId = snapshot.activeApplicationId
          contributions.clear()
          for (const [id, entry] of next) contributions.set(id, entry)
          const active = snapshot.activeApplicationId && contributions.has(snapshot.activeApplicationId)
            ? snapshot.activeApplicationId
            : null
          publish(active)
        },
        rollback: () => {
        ...
        revert: () => {
          if (!committed || !previousEntries || previousActiveApplicationId === undefined) {
            throw new Error(`Application transaction 尚未 commit：${owner.key}`)
          }
          if (reverted) return
          reverted = true
          contributions.clear()
    ...
    mount(applicationId) {
      if (!contributions.has(applicationId)) {
        throw new Error(`Application 未注册：${applicationId}`)
      }
      if (snapshot.activeApplicationId === applicationId) return
      publish(applicationId)
    },
    unmount() {
      if (snapshot.activeApplicationId === null) return
      publish(null)
    },
    resolve(applicationId) {
      return contributions.get(applicationId)?.contribution ?? null
    },
    getSnapshot() {
      return snapshot
    },
