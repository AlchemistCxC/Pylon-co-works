...
impl PetState {
    ...
    pub fn restore(mut saved: Self, now_ms: u64) -> Self {
        ...
        // I18 W4（LR2-WI09）：去重 + 诊断保留——损坏存档重复 id 会错误触发
        // unlock_achievements 的 len≥表长 early-exit（漏解锁）；剔除未知 id 时 warn 保留诊断。
        let before_unlocked = saved.unlocked.len();
        saved
            .unlocked
            .retain(|id| ACHIEVEMENTS.iter().any(|a| a.id.as_str() == id.as_str()));
        if saved.unlocked.len() != before_unlocked {
            // pet-core 不依赖 tracing——诊断走 stderr（保留未知字段剔除信息，不静默吞）
            eprintln!(
                "pylon-pet restore: {} 个未知成就 id 被剔除",
                before_unlocked - saved.unlocked.len()
            );
        }
        let mut seen_unlocked = std::collections::HashSet::new();
        saved.unlocked.retain(|id| seen_unlocked.insert(id.clone()));
        ...
        // I18 W3（LR2-WI09）：restore 视为「苏醒」——last_activity_at_ms 置 now 而非 0，
        // 否则首轮轮询 check_sleepy 按 0 起算空闲 → 立即入睡，与「又亮起来了」文案冲突。
        saved.last_activity_at_ms = now_ms;
    ...
    fn visit(&mut self, now_ms: u64) {
        ...
        // I18 W4（用户 2026-08-12 拍板「loneliness 增长」）：缺席 → loneliness 状态增长
        // （对齐设计书 §6：loneliness>60 触发 lonely 情绪、>70 孤单语音），不再扣 happiness。
        // 旧实现 `happiness -= loneliness` 让缺席只降快乐且 loneliness 字段恒 0（永不显 lonely）。
        let loneliness_gain = (elapsed.min(5) * VISIT_LONELY_PER_DAY) as u8;
        self.loneliness = self.loneliness.saturating_add(loneliness_gain).min(100);
