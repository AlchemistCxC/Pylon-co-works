...
#[tauri::command]
pub(crate) async fn list_runtime_logs(state: tauri::State<'_, AppState>, query: Option<runtime_log::RuntimeLogQuery>) -> Result<Vec<runtime_log::RuntimeLogEntry>, PylonError> {
    Ok(state.runtime_logs.list(&query.unwrap_or_default()))
}

#[tauri::command]
pub(crate) async fn clear_runtime_logs(state: tauri::State<'_, AppState>) -> Result<(), PylonError> {
