...
pub(crate) struct CatalogInvocation {
    pub command: String,
    pub args: Vec<String>,
}
...
pub(crate) enum CatalogConfigFormat {
    Json,
    Yaml,
}
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct CatalogConfigEvidence {
...
pub(crate) enum CatalogSetModelApi {
    ConfigOption,
    SetModel,
    None,
}
...
#[derive(Debug, Clone)]
pub(crate) struct AgentDetectionProfile {
...
pub(crate) fn detection_profiles() -> Result<Vec<AgentDetectionProfile>, String> {
...
pub(crate) fn provider_for_executable_stem(stem: &str) -> Result<Option<String>, String> {
...
pub(crate) fn set_model_api_default(provider: &str) -> Result<Option<CatalogSetModelApi>, String> {
    Ok(catalog()?
        .providers
        .iter()
