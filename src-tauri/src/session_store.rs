...
pub(crate) fn update_if_current<T>(
    ...
    let session = sessions
        .get_mut(source)
        .ok_or_else(|| SessionStoreError::Stale(format!("stale session mapping for source: {source}")))?;
...
mod tests {
    ...
    fn session(id: &str, generation: u64) -> SessionInfo {
        SessionInfo::new(id.to_string(), "persona".to_string(), ".".to_string(), true, generation)
    }

    #[test]
    fn insert_replaces_same_source_and_notifies_mapping_ready() {
        let rt = runtime();
        let replaced = insert(rt.as_ref(), "src", session("peri-1", 0), true, 100)
            .expect("first insert");
        assert!(replaced.is_none(), "首次插入无旧映射");
        let replaced = insert(rt.as_ref(), "src", session("peri-2", 0), true, 100)
            .expect("same source replace");
        assert_eq!(replaced.unwrap().peri_id, "peri-1", "同 source 替换返回旧映射");
        assert_eq!(snapshot(rt.as_ref()).unwrap().len(), 1, "同 source 替换不增加条目");
    }
    ...
    fn migrate_or_clear_keeps_fields_and_clears_on_false() {
        ...
        assert!(snap.iter().all(|(_, s)| s.generation == 9), "generation 必须迁移");
