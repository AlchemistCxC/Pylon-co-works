...
use crate::gateway::instance::InstanceNotifier;
...
/// - 健康连接（>60s）清零累计。
fn decide_quick_reconnect(
    duration_secs: f64,
    close_code: u16,
    quick_count: u32,
) -> (ReconnectOutcome, u32) {
...
/// cancel 感知睡眠：cancel 触发返回 false（调用方应收敛退出）。
async fn cancellable_sleep(cancel: &Option<CancellationToken>, delay: Duration) -> bool {
    match cancel {
        Some(c) => tokio::select! {
            _ = c.cancelled() => false,
            _ = sleep(delay) => true,
...
/// 应用一次退避延迟后睡眠（cancel 感知）；退避耗尽（超过 MAX_RECONNECT_ATTEMPTS 次）
/// 或 cancel 触发返回 false。
async fn wait_backoff_cancellable(
...
/// WS 事件循环（B10.2 原入口；lib.rs run() 接线）：无 cancel/notifier，
/// 循环随 tokio runtime 终止（桌面进程退出即清理）。
pub async fn run_ws_loop(http_client: Client, auth: Arc<QqAuth>, adapter: Arc<QqAdapter>) {
    let _ = run_ws_loop_inner(http_client, auth, adapter, None, None).await;
}

/// 托管 WS 事件循环（W2 QQ factory run future）：cancel 感知 + 状态上报 +
/// 收敛清理。stop 触发 cancel 后：中断连接/退避 → 关闭适配器（T12-4）→
/// 返回 Ok（watchdog finalize 收敛 Stopped，不留后台残余）。
pub async fn run_managed_ws_loop(
    http_client: Client,
    auth: Arc<QqAuth>,
    ...
    run_ws_loop_inner(http_client, auth, adapter, Some(cancel), Some(notifier)).await
}

/// WS 事件循环实现：连接失败/断开按 close code 分类重连（指数退避 2s→60s，
/// 上限 100 次；3 次快速断开视为凭证问题停止）。cancel 非空时：
/// - 连接/退避期间 cancel 触发立即收敛（shutdown 适配器后返回 Ok）
/// - 连接就绪（READY）经 notifier 上报 Connected（mark_connected 幂等）
/// - 致命 close code 上报 Error 并返回 Err（watchdog finalize 落 Error）
async fn run_ws_loop_inner(
    http_client: Client,
    auth: Arc<QqAuth>,
    adapter: Arc<QqAdapter>,
    cancel: Option<CancellationToken>,
    notifier: Option<InstanceNotifier>,
) -> Result<(), String> {
    ...
    loop {
        // W2 T12-4：cancel 已触发 → 关闭适配器、收敛退出（stop 不留后台残余）。
        ...
        match outcome {
            ...
            Err(ref e) if e.contains("op 9") => {
                ...
                if op9_streak >= OP9_DEGRADE_THRESHOLD {
                    ...
                    if !wait_backoff_cancellable(&cancel, &mut backoff).await {
                        adapter.shutdown();
                        // I12 W8（LR2-WI04）：主动 stop（cancel 触发）→ Ok（收敛 Stopped）；
                        // 退避耗尽（超过 MAX_RECONNECT_ATTEMPTS）→ Err（last_error 可见，实例 Error）
                        if cancel.as_ref().is_some_and(|c| c.is_cancelled()) {
                            return Ok(());
                        }
                        return Err("QQ WS: 重连耗尽（超过最大尝试次数），连接终止".to_string());
                    }
                } else {
                    backoff = reconnect_backoff();
                    quick_count = 0;
            ...
            Err(e) => {
                ...
                let (outcome, new_quick_count) =
                    decide_quick_reconnect(duration, code, quick_count);
                if duration > 60.0 {
                    backoff = reconnect_backoff();
                }
                ...
                match outcome {
                    ReconnectOutcome::Stop(desc) => {
                        ...
                        if let Some(notifier) = notifier.as_ref() {
                            notifier.failed(desc.to_string()).await;
                        }
                        adapter.shutdown();
...
async fn run_connection(
    ...
    on_ready: Option<&InstanceNotifier>,
) -> Result<(), String> {
                            ...
                            Message::Text(text) => {
                                match serde_json::from_str::<QqEvent>(&text) {
                                    ...
                                    if op == 0 && t == "READY" {
                                        ...
                                        // W2：连接就绪（服务器已接受 Identify/Resume）→
                                        // 上报 Connected。每次重连均上报——mark_connected
                                        // 幂等，仅 Starting→Connected 生效。
                                        if let Some(notifier) = on_ready {
...
mod tests {
    ...
    #[tokio::test]
    async fn managed_loop_exits_ok_on_cancel_before_any_connect() {
        // W2 T12-4：token 先触发再 poll → 首轮循环即 shutdown 收敛并返回 Ok，
        // 不触碰网络（不悬挂、不留后台残余）。notifier 传 None（InstanceNotifier
        // 字段私有不可在 qq 模块构造；cancel 前无连接、不触发任何上报）。
        let core = Arc::new(crate::gateway::GatewayCore::new());
        let auth = Arc::new(QqAuth::for_testing("test-token".to_string()));
        let adapter = QqAdapter::new(core, Client::new(), auth.clone());
        let cancel = CancellationToken::new();
        cancel.cancel();
        tokio::time::timeout(
            Duration::from_secs(2),
            run_ws_loop_inner(Client::new(), auth, adapter.clone(), Some(cancel), None),
        )
        .await
        .expect("cancel 后 run 应立即退出，不得悬挂")
        .expect("cancel 收敛为 Ok");
        // 收敛后适配器已关闭（shutdown 由内层循环调用）——出站投递必须拒绝。
        use crate::gateway::PlatformAdapter;
        assert!(
            adapter.deliver_text("qq:group:g1", "hi").is_err(),
            "收敛后适配器必须已关闭（投递拒绝）"
        );
    }
}
