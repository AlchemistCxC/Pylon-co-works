...
fn classify_send_error(error: &str) -> SendFailure {
    ...
    // 其余 Transient。
    // I12 W8（LR2-WI04）：401/403/forbidden/无权限 必须先于「失效」匹配——令牌失效错误
    // 形如 `HTTP 401: token 已失效` 含双特征，误判 DeadTarget 会进入死目标 TTL 短路投递
    // （ISSUE-12 问题 5）。
...
impl QqAdapter {
    ...
    pub fn handle_incoming(
        ...
        // W2 T12-4：适配器已关闭 → 丢弃新入站（stop 收敛期 WS 残余帧不 ingest）。
...
impl PlatformAdapter for QqAdapter {
    ...
    fn deliver_text(&self, source: &str, text: &str) -> Result<(), String> {
        // W2 T12-4：适配器已关闭 → 拒绝新投递（注销出站入口）。
        if self.closed.load(Ordering::SeqCst) {
            return Err("QQ 适配器已关闭".to_string());
        }
...
impl QqAdapter {
    /// 关闭适配器（W2 T12-4 收尾：注销新投递 → cancel 发送 worker → 清引用）。
    /// 置 closed 后：deliver_text 拒绝、handle_incoming 丢弃（见两处入口短路）；
    /// 取消全部发送 worker token 并清空 senders——channel 随 sender 句柄 drop
    /// 关闭，send_loop 的 recv() 收到 None 后退出（在飞 HTTP 发送有限 drain）。
    /// 由托管连接循环在 cancel/致命错误收敛时调用；幂等，可安全重复调用。
    pub(crate) fn shutdown(&self) {
        self.closed.store(true, Ordering::SeqCst);
        if let Ok(mut senders) = self.senders.lock() {
            for worker in senders.values() {
                worker.token.cancel();
            }
            senders.clear();
        }
...
mod tests {
    use super::*;
    use crate::test_utils::spawn_http_stub;
...
"#;
    ...
    #[tokio::test]
    async fn shutdown_closes_deliver_and_ingest() {
        // W2 T12-4：shutdown 后出站拒绝、入站丢弃（不触网、不 dispatch）。
        let adapter = test_adapter(Arc::new(GatewayCore::new()));
        adapter.shutdown();
        assert!(
        ...
        assert!(
            adapter
                .handle_incoming("qq:group:g1", "m1", "hi", None, None)
                .unwrap()
                .is_none(),
            "shutdown 后入站必须丢弃"
        );
    ...
    #[tokio::test]
    async fn shutdown_cancels_sender_workers_and_clears_map() {
        // T12-4「关闭发送入口 + cancel workers」：token 触发 + senders 清空 →
        // channel 随 sender 句柄 drop 关闭 → send_loop recv() None 退出。
        let adapter = test_adapter(Arc::new(GatewayCore::new()));
        let (tx, _rx) = mpsc::channel(SEND_QUEUE_CAP);
        let token = CancellationToken::new();
        adapter.senders.lock().unwrap().insert(
            "group:g1".into(),
            SendWorker {
                tx,
                token: token.clone(),
        ...
        adapter.shutdown();
        assert!(token.is_cancelled(), "shutdown 应取消发送 worker token");
        assert!(
            adapter.senders.lock().unwrap().is_empty(),
            "shutdown 应清空 senders（channel 关闭 → send_loop 退出）"
        );
    }
...
"#;
    ...
    /// 顺序响应桩收敛至 crate::test_utils::spawn_http_stub（同构语义）。
    ...
    async fn rate_retry_dead_target_failure_is_marked() {
        ...
        // 真正的死目标语义由 404 承担）。
        let (address, _request_rx, server) = spawn_http_stub(&[
    ...
    fn classify_send_error_matches_chinese_and_http_status() {
        ...
        // I12 W8（LR2-WI04，RED 前）：令牌失效错误含「401」与「失效」双特征——
        // 401/403 必须优先于「失效」，否则误判 DeadTarget 进入死目标 TTL 短路投递。
        assert!(matches!(
            classify_send_error("HTTP 401: token 已失效"),
            SendFailure::AuthOrPermission(_)
        ));
        assert!(matches!(
            classify_send_error("HTTP 403: 凭证已失效"),
            SendFailure::AuthOrPermission(_)
        ));
        assert!(matches!(
