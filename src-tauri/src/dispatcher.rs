...
use crate::agent_runtime::{session_mapping_matches, source_for_peri_id_in_generation, AgentLifecycleStatus};
...
pub(crate) fn start_notification_dispatcher<R: tauri::Runtime>(handles: &AppStateHandles, runtime: &Arc<AgentRuntime>, window: tauri::WebviewWindow<R>) {
    if let Ok(mut task) = runtime.notification_task.lock() {
        ...
        let generation = runtime.client_generation.load(std::sync::atomic::Ordering::Acquire);
        ...
        *task = Some(tokio::spawn(async move {
            ...
            loop {
                    ...
                    if runtime_for_reconnect.auto_reconnect_active.swap(true, Ordering::AcqRel) == false {
                        ...
                        tokio::spawn(async move {
                            ...
                            for attempt in 1..=agent_runtime::MAX_RECONNECT_ATTEMPTS {
                                tokio::time::sleep(std::time::Duration::from_millis(agent_runtime::reconnect_backoff_ms(attempt))).await;
                ...
                if raw.method.as_deref() != Some(crate::acp::NOTIF_SESSION_UPDATE) { continue; }
            ...
            Some(crate::acp::SessionUpdateVariant::ToolCall) => {
                                ...
                                let _ = pet.lock().map(|mut p| crate::pet::on_tool_started_kind(&mut p, kind));
                                ...
                                if let Some(raw) = update.get("rawInput").and_then(|v| v.as_str()) {
                                    if let Some(file) = extract_tool_file_name(raw) {
                                        let _ = pet.lock().map(|mut p| crate::pet::record_code_file(&mut p, &file));
                                    }
                                }
                            }
                            Some(crate::acp::SessionUpdateVariant::ToolCallUpdate) => {
                                match update.get("status").and_then(|v| v.as_str()) {
                                    ...
                                    Some("cancelled") => { let _ = pet.lock().map(|mut p| crate::pet::on_tool_cancelled(&mut p)); }
                                    _ => {}
                                }
                            }
                                ...
                                if let Some(options) = update.get("configOptions").and_then(|v| v.as_array()) {
                                    ...
                                    match (option_key, current) {
                                        (Some("model"), Some(model)) => {
                                            ...
                                            if changed {
                                                let _ = pet.lock().map(|mut p| crate::pet::on_model_changed(&mut p, &model));
                                            }
                                        }
                                        (Some("mode"), Some(mode)) => {
                                            ...
                                            if changed {
                                                // M5 感知：工作模式切换
                                                let _ = pet.lock().map(|mut p| crate::pet::on_mode_changed(&mut p, &mode));
                                            }
                                        }
                                        _ => {}
                                    }
