...
pub(crate) fn parse_permission_request(params: Option<&serde_json::Value>) -> Option<PendingPermission> {
    let params = params?;
...
/// 供重试/超时/客户端替换清理）。
pub(crate) async fn respond_permission(runtime: &AgentRuntime, request_id: u64, response: serde_json::Value) -> bool {
...
pub(crate) async fn respond_pending_permissions_cancelled(runtime: &AgentRuntime, session_id: &str) {
...
#[tauri::command]
pub(crate) async fn approve_tool_call(state: tauri::State<'_, AppState>, request_id: u64, option_id: String) -> Result<(), PylonError> {
...
#[tauri::command]
pub(crate) async fn set_approval_mode(state: tauri::State<'_, AppState>, mode: String) -> Result<(), PylonError> {
