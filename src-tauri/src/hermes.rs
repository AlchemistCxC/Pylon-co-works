...
//! `-p` 参数**，因此 Hermes 走 active_profile 机制（当前机器为 `profile-x`，其
//! provider=root config 的 `nous`），而可用 profile `profile-a`（deepseek +
...
fn parse_hermes_home(content: &str) -> Option<String> {
    for line in content.lines() {
        ...
        if let Some(value) = rest {
            ...
            // bash 形态 `HERMES_HOME="D:/hermes" exec ...`：取第一个引号对内的
            // 内容或第一个空白前的 token（Windows 路径可含空格，引号对优先）。
            let parsed = if value.starts_with('"') {
                value.split('"').nth(1).map(|token| token.to_string())
...
mod tests {
    ...
    fn parses_hermes_home_from_bash_script() {
        let content = "#!/usr/bin/env bash\nHERMES_HOME=\"D:/hermes\" exec python hermes \"$@\"\n";
        assert_eq!(parse_hermes_home(content).as_deref(), Some("D:/hermes"));
    }
    ...
    fn absolute_profile_path_is_used_verbatim() {
        let agent = agent_with_profile(Some("D:/hermes/profiles/profile-a"));
        let resolved = resolve_profile_dir(&agent, None).expect("absolute path");
        assert_eq!(resolved, PathBuf::from("D:/hermes/profiles/profile-a"));
    }

    #[test]
    fn relative_profile_path_resolves_against_base_dir() {
        let agent = agent_with_profile(Some("profiles/profile-a"));
        let resolved =
            resolve_profile_dir(&agent, Some(Path::new("C:/apps/pylon"))).expect("relative path");
        assert_eq!(resolved, PathBuf::from("C:/apps/pylon/profiles/profile-a"));
    }
