...
fn config_transaction_is_cas_and_backup_is_previous_exact_bytes() {
    ...
    let error =
        write_config_transaction(&path, &baseline, third).expect_err("stale writer must conflict");
...
fn embedded_repository_config_can_materialize_default_switch() {
    ...
    let agents =
        validate_candidate(&patched, None).expect("repository config candidate must remain valid");
    assert_eq!(
        default_agent_id(&agents).unwrap(),
...
fn agent_field_patch_rejects_unknown_and_bad_types() {
    ...
    assert!(
        apply_agent_field_patch(content, "a", &serde_json::json!({ "transport": "http" })).is_err()
    );
    assert!(
