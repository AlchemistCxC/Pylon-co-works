...
- [kumo] #372+#371 发行包清偿与离线文档站（两个施工 agent 按文件域互斥并行）：
  - #372（打包与模板）→ `resources/release/**`、`scripts/pack_release.py`、`scripts/tests/test_pack_release.py`、`docs/说明书/{Pylon-发行包清单,Pylon-项目架构参考}.md`、`src-tauri/src/agent_config/{embedded_agents.yaml,load.rs}`：README.txt 的 Hermes 段失实修正 / 下线 `repair-hermes-acp.{bat,ps1}` / 零 Agent 的 `agents.yaml` 模板化（**不得推翻 #326**）。
  - #371（离线文档站）→ `docs/.vitepress/**`（离线 base + 字体裁剪）、`package.json`（发行链加离线构建）、**`scripts/pack_release.py` 的 dist 收集归 #372 的 agent**（契约：`docs/.vitepress/dist` → 包内 `docs-site/`，缺则 PackError）、`src-tauri/tauri.conf.json`（CSP+resources）、`src-tauri/src/lib.rs`（注册 docs scheme）、前端专用文档 Sheet。
  - 规格 `.agents/spec/release-package-cleanup.md`、`.agents/spec/offline-docs-site.md`。
  - 实测：`bun run docs:build` dist = 26 MB，其中 Web 字体约 22 MB（`theme/index.ts` 的 source-serif-4 + noto-serif-sc）→ 离线变体裁字体后应降至约 4 MB。
  - **两边都不碰**：`pylon-session/**`、`src-tauri/pylon-acp/**`。
  - 附带：`AGENTS.md` §2.5 新增「推送 PR 必须逐条写清 issue 操作（是否彻底关闭）」；#367 裁决「暂不做、保留决策口」已回写。
