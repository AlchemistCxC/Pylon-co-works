...
- 前端 issue②：`src/infrastructure/events/canonicalEventFeed.ts` + `src/__tests__/replay/canonicalEventFeed.test.ts`（canonical feed 缺 `pylon:update` 广播兜底）
- 前端 issue③（#309 覆盖层让位）：`src/app/modalOverlayStore.ts`（新增）、`src/App.tsx`、`src/components/{PermissionDialog,SessionOwnerRecoveryDialog,sidebar/SessionsPanel}.tsx`、`src/sheets/browser/BrowserSheetView.tsx`、`src/sheets/browser/__tests__/BrowserSheetView.modalOverlayVeil.test.tsx`、`src/app/__tests__/modalOverlayStore.test.tsx`
- `src/sheets/file/**` 与 `src/application/hooks/**` 属他人在途域，本轮**未触碰**
