...
## 修订 2（2026-09-22）· 解析改为按时间切片并让出主线程

**状态**：已生效（同日落地，issue #241；用户「现在做」）。

### 取代了什么

修订 1 记的修法是 `ensureSyntaxTree(state, code.length, 200ms)`——它解决了丢色，但留下
「超过 200ms 预算（约 45 万字符）仍退回部分树」的**静默降级**。本修订把它做掉。

### 最终形状（只用 Lezer 公开 API）

`Parser.startParse(code, [])` → 空转 `advance()` 直到返回树；`advance()` 每次约 1µs、未完成返回
`null`，于是按时间片让出是自然的（8ms 一片，与 #221 调度器预算同量级）。让出实现按运行时能力选
`scheduler.yield()` → `MessageChannel` → `setTimeout(0)`。

三条更省事的路都实测不可行，记下来省后人一次弯路：`ensureSyntaxTree` 反复调用**不累积**
（每片烧完预算、树长不动）；`context.work()` 能累积但 `work`/`isDone`/`tree` **不在
`ParseContext` 的公开声明里**；`startParse` + `TreeFragment` 跨片复用**直接抛异常**。

等价性已验：新路径与走 `Language` machinery 的结果**逐区间完全一致**（ts 9888 符文、
html 529 符文——html 是唯一带嵌套语言 `parseMixed` 的受支持语言，故以其为最严判据）。
根因清楚：CM 的 `Language.state` 内部用的就是同一个 `support.language.parser.startParse`。

### 顺带拿到的两笔

1. **不再有重复解析**：旧路径里 `EditorState` 的 `Language.state` 字段初始化先同步解析 3006 字符，
   随后 `ensureSyntaxTree` 为到达目标又从头解析 ⇒ 前 3006 字符解析两遍。单次 `startParse` 只解析一遍。
2. **每次调用的动态 import 归零**（`highlightTree` 并入引擎缓存；`@codemirror/state`、
   `@codemirror/language` 的运行时 import 随 `EditorState` 一起去掉）。
...
| 500,000 | 201ms 后**截断到 3001 字符** | **235ms 全解析**（跨帧让出） |

⇒ 「单次 4KB 高亮 1.13ms」那个数字（修订 1 已说明它是截断量出来的）在最终实现下是 **4k ≈ 2.05ms**；
`pl-*` 覆盖率 47.7% → 69.4% 的结论与决策均不变。

### 对原决策的影响

**无。** 决策仍由内存驱动；本修订只把「大块怎么办」从「截断」改成「切片 + 让出」。
新增的代价是**没有总量上限**：切片只限制单次不让出的时长，总量随规模线性增长（1MB ≈ 0.47s，
分摊到帧）。这是有意的——任何形式的「算一半」都是回到静默降级。
