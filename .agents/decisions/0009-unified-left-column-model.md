...
5. **折叠按钮固定在标题栏最左端，三灯在其右**（即左格首位，顺序固定）。
   - 位置在折叠前后**不变**：标题栏 grid 第 1 列取 `max(轨道宽, 折叠按钮宽)`（按钮宽 42px，记为 `--titlebar-rail-toggle-width`）。展开时该列 = 轨道宽（240）；折叠时收窄到 42px 而不是 0，因此按钮始终停在 x=0。
   - 这**不是**「折叠后留空列」：格子里的内容是按钮本身（用户反对的是 Agent sheet 折叠后的空列与 File 折叠后的图标条，那是**正文区**的残留宽度，正文左列仍为 0）。
   - 不能放右侧应用控制簇：会挤动「右侧栏 / 界面 / 设置」与窗口控制的落点，而三者位置必须恒定（用户明确要求）。不能把左格 `display:none`：移除一个 grid item 会让后两个兄弟自动前移一列，右侧两簇整簇跳到窗口最左侧（实测菜单 957/997/1037 → 4/44/84、窗口按钮 1086/1124/1162 → 133/171/209）。
   - 按钮**始终渲染**（本 Sheet 无左栏时禁用），避免随 Sheet 能力忽隐忽现；图标表达「左栏在/不在」（`PanelLeftClose`/`PanelLeftOpen`；glyph 皮肤用 `▤`/`▢`）而不是读成「打开菜单」的 ☰。折叠时只隐藏三灯。
