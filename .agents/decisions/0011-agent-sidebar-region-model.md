...
22. **拖拽改为长按模块头，删掉独立手柄**（用户修正）：手柄方案有两个代价——常驻一个抓取图标是噪声（与「折叠按钮太显眼」同一类问题），而按需显形就必须给它 `visibility/pointer-events` 门控，否则是个看不见却能拖的靶子（本轮先实现了门控，随即被用户否掉整个方案）。长按（260ms、移动超 6px 取消）把手势与「点击标题」区分开，模块头因此可以完全干净；拖拽抬起后 320ms 内忽略 click，避免连带触发展开/进页面。
23. **统一左轨 `--sidebar-rail-pad`（默认 6px）**（用户修正「工作区缩进不正常，期望与模块图标一致」）：模块头、工作区组头、搜索行、会话列表缩进全部由它派生；图标占位统一 15px，使**图标与标题**都落在同一列。实测模块图标 / 工作区图标 / 搜索图标同在 x=6，标题同在 x=26。两处坑：终端模式给 `.session-list` 自带 6px 内边距（`.app[data-interface-mode=...]` 特异性更高，须显式归零），以及 `.cwd-group` 的 1px 透明边框（toggle 侧减 1px 补偿）。
24. **搜索从「框」改为内联行**（用户修正「目前的框已经不适合 ui 界面了」）：左栏已是 26px 无框行构成的紧凑列表，一个 36px 描边输入框夹在中间像外来控件。改为 `图标 + 无框输入`，hover 提亮、focus-within 转强调色；旧 `.search-input` 规则（已无消费方）删除。
25. **移除工作区组右侧的会话计数显示**（用户要求）：`.cwd-group-count` 及其交叉淡出规则一并删除，组右侧只剩 hover 显形的动作按钮（同一套 opacity + visibility/pointer-events 门控）。

26. **拖拽抖动（用户实机报「疯狂抖动」）的根因与修法**：原实现是「实时重排预览」——pointermove 里直接改渲染次序，于是重排把被拖模块挪出光标 → 目标位置按新布局重算 → 又挪回去，形成反馈环。改为**长按那一刻冻结各模块头的中心线**，落点只由冻结几何决定，预览改用一条落点指示线，抖动在结构上不可能再发生。
...
31. **`alwaysOpen` 追加「钉在栈底」**（用户要求「会话相关的与其余模块搞一个分界，并且始终位于模块最下方」）：常驻模块不再参与排序——不响应长按拖拽，其它模块的落点被钳在钉区之前（`dropIndexAt` 取 `min(index, pinnedStart)`），次序收纳（`applyModulePrefs`）把钉区强制排在末尾，因此旧偏好里把常驻模块排在前面的值也会被纠正。改动前 `order: 900` 只是**初次**次序，一次拖拽就能把会话挪到顶上。**分界形态**（不画横线）经用户在三案中裁定为 **B**：留白（栈 gap 6px + 常驻区 margin-top 4px = 10px）+ 常驻区整块 `--bg-panel` 层底 + 标题 `position:sticky` 吸顶（吸顶头用近不透明的 `--surface-panel` + `backdrop-filter:blur(8px)`，否则列表行会从标题底下透出字影）。**任何 border-top / 伪元素分隔线都不许出现**——这条由 CSS 契约测试钉住（用户原话「不必搞成横线」）。
