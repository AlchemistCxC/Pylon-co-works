...
31. **`alwaysOpen` 追加「钉在栈底」**（用户要求「会话相关的与其余模块搞一个分界，并且始终位于模块最下方」）：常驻模块不再参与排序——不响应长按拖拽，其它模块的落点被钳在钉区之前（`dropIndexAt` 取 `min(index, pinnedStart)`），次序收纳（`applyModulePrefs`）把钉区强制排在末尾，因此旧偏好里把常驻模块排在前面的值也会被纠正。改动前 `order: 900` 只是**初次**次序，一次拖拽就能把会话挪到顶上。**分界形态**（不画横线）另行决定，与本条无关。
