...
## 追加（2026-09-22）：未接线 wasm 出口 vs 现役实现的性能对照

用户当轮指令：**「临时写测试，多轮次去最大最小取平均值，测 wasm 死实现与现有实现的性能差距，
高于现有实现则转正且退伍现有实现」**。本节是那次测量的完整证据。

**临时脚本已删**（一次性判定用的 scratch，形状写在下面；与记录 220 / #205 的 scratch 同处置）。
判据：`bun` 直跑 node 宿主；两侧**同一份工作**；估计量为**配对比值**——每轮两侧相邻各测一次
（奇数轮 A→B、偶数轮 B→A 抵顺序偏差），对该轮 `死/现` 比值去一最大一最小后取平均；
另报比值的 min/median/max 与「最小/最小」（两侧各自 clean floor 的比）作为第二估计量；
`scopeForLanguage` 那类纯 JS 查表把结果折进 sink 累加器并在末尾打印（否则 V8 会把整个循环消掉——
实测没加 sink 时读出 `0.5ns/次`，比一次 `toLowerCase` + 属性查表还快一个数量级，是假读数）。
可比性先检查：两侧输出键序归一后必须逐字节一致（`*Json + JSON.parse` vs 结构化返回实测等价）。

**三次独立运行（25 / 31 / 25 轮）**：

| 对照 | 死/现（截尾均值）三次 | 最小/最小 | 判定 |
| --- | --- | --- | --- |
| `scopeForLanguage`：wasm 出口 vs 生产 TS 表 | **17.99 / 18.14 / 18.08** | 19.50 | **现役（TS）快 ~18×，稳定** |
| `parseMarkdownJson + JSON.parse` vs 结构化 · `shape-heading`(4k) | 0.810 / 0.820 / 0.772 | 0.795 | 死名义快 ~20%，**比值 max 跨 1（1.10 / 1.21）⇒ 不可靠** |
| 同上 · `shape-table`(4k) | 0.896 / 0.884 / 0.963 | 0.938 | 跨 1 ⇒ 不可靠 |
| 同上 · `shape-fence`(4k) | 0.978 / 1.047 / 0.947 | 0.962 | 方向翻转 ⇒ 无差异 |
| 同上 · `doc-m`(63.6k) | 0.819 / 0.797 / 0.835 | 0.798 | 死名义快 ~20%，但比值 max 跨 1（1.28 / 1.42）⇒ 不可靠 |
| 同上 · **`tail-320`（生产热路径形状）** | 1.176 / 1.245 / 1.240 | 1.178 | **现役快 ~20%** |
| 同上 · **`tail-1280`（生产热路径形状）** | 1.441 / 1.300 / 1.276 | 1.245 | **现役快 ~28%**，两个估计量一致 |
| `highlightBlockJson + JSON.parse` vs 结构化（6 档：css/ts/go/4k/40k/200k） | 全部 0.91–1.11 | 0.87–1.02 | **无差异**（40k 的最小/最小 0.87 略偏死，但截尾在三轮里 1.006 / 1.001 / 0.907 ⇒ 噪声） |
| `splitStreamingMarkdownBlocks` vs `splitStreamingMarkdownBlockEnds`（**不可比**） | **2.13 / 2.28 / 2.38**（m）、**2.51 / 2.41 / 2.51**（l） | 2.39 / 2.35 | **现役（ends）快 ~2.4×，三轮一致** |
| `splitStreamingMarkdown` vs ends（**不可比**） | 1.11 / 1.06 / 1.09（m）、1.33 / 1.17 / 1.09（l） | 1.04 / 1.16 | 现役名义快 ~10%，跨 1 |
| `findLastStableBlockBoundary` vs ends（**不可比**） | 0.79 / 0.81 / 0.80（m）、0.94 / — / 0.87（l） | 0.76 / 0.78 | 死名义快 ~15%，跨 1；且**工作量远小**（只返回一个数） |

**判定：没有任何一对满足「死实现高于现役实现」且可靠 ⇒ 不转正、不退伍。**

三条要点（避免后人误读这张表）：

1. **两个可靠信号都指向现役更优**：`scopeForLanguage` 的 wasm 出口比生产 TS 表慢约 **18×**；
   `splitStreamingMarkdownBlocks` 比现役 ends 出口慢约 **2.4×**（三轮一致）。后者顺带**印证了
   #220 边界收口的取舍**——把整组块字符串过界换成 u32 偏移数组确实更便宜。
2. **`parseMarkdownJson` 的「大文档更快」不足以支撑转正**：它在 ≥4k 文档上可复现地快约 20%
   （方向在三次运行里一致），但在**生产真正高频的形状（每帧增长的短尾）上慢约 20–28%**，
   而短尾才是 `markdownRenderModel` 的调用面（stable 块走 LRU 复用、不重解析）。
   一个直换会拿热路径的损失去换冷解析的收益，是净负。且那个 20% 的「赢」本身比值 max 跨 1。
3. **三对切分出口根本不适用这条规则**：`ends` 返回块结束偏移数组，另外三个返回块字符串／
   单个边界数——消费方不是同一个（`MarkdownContent.deriveRowSpecs` 要的是全部块偏移才能逐行切片）。
