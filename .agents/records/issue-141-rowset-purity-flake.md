...
无。遗留观察（不影响本 issue 结论、未施工）：增长尾块以 `{ cache: false }` 绕 LRU，token 级切片下会为每个中间态各起一次解析（本用例约 900 次），浪费的解析在并发负载下会拉长落地时间。若要根治「解析落地慢」，那是 `MarkdownContent.solid.tsx` / `markdownRenderModel.ts` 的独立议题（去抖或丢弃已被取代的解析），需另开 issue 与 spec。
