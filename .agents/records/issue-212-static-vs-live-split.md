...
1. **渐进挂载行（S3b-3）**：整发之后，冷开长会话会把一屏新行在同一次发布里全部首次解析（实测约 4.5ms/千字符）。是否需要在 `PlainMessageList` 上加"首 16 行 + 每帧 +32"的挂载窗口，应由真机读数决定（long task 数与首屏可见时间）。
2. **流式代码块纳入折叠（#208 遗留）**：`StreamingCodeBlock` 目前逐行渲染全部内容，`SolidCodeBlock` 的折叠/展开入口在流式期到不了。设计上有一个真实分叉，需用户裁决：流式期折到**头部**（与settle 后一致，但看不到新生成的代码）还是显示**尾部窗口**（能看到新内容，但 settle 时会从尾部跳到头部）。
3. **真机复验**：几何与节奏类断言（打开不抖、上滚阅读不被挤走）在 jsdom 里没有布局，单测只能钉判据与补偿算法本身。
