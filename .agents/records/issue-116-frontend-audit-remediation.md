...
6. **统一回归项要求的「截图比对」以数值比对替代**。逐页采集的是 `settings-body` 的 `scrollWidth/clientWidth`、超宽元素清单与计数、关键元素的盒模型与计算样式（同脚本、修复前后各一遍），而不是像素级截图 diff。理由：被测页面含实时数据（会话时间、探测耗时、渲染器 fixture 滚动位置），像素比对不可复现；而「无新增横向滚动 / 无新增裁切」本身就是可数值判定的命题，逐元素计数比目视更严格。目视核验另行做了两页（模板库、渲染器）——卡片的缩略图/按钮/名称已成整体、字段行四者同行。
7. **门禁补强未做**（spec 文档同步项 8）：为 P93 验收补「计算样式断言」超出本 issue 范围，本轮只补了源码文本级的层叠契约测试。
