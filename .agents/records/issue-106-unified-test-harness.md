...
- 诊断修正：scheduler 失败来自误置 jsdom（`requestAnimationFrame` 改变调度路径），新增 `solid-logic` Node project 后 27 项全绿；Solid 组件仍单独 jsdom project。全量 projects 仍暴露若干既有测试对工作目录/共享 fake invoke 的隐含依赖（557 文件、3647 测试中 3601 通过），需继续按测试契约清理。
- P4 验收：按每个测试文件的环境 docblock 与模块 mock 使用情况生成 `node-shared`、`node`、`react-shared`、`react-dom`、`solid-dom` 五个 project；纯 Node 无 mock 测试共享进程，其余存在异步 DOM/mock 生命周期的分组保持 isolate。setup 在文件结束恢复 mocks、globals、timers 与模块缓存。全量 `vitest run`：3789/3789 通过，0 失败。
