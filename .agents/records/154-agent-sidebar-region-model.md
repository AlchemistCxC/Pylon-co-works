...
**门禁**：`tsc -b` 无输出；`eslint` 0 error；全量 **601 文件 / 4393 用例通过**。

**实机数值**（dev server + mock 数据；首轮量不到行是因为 **dev server 的模块图陈旧**——`bunx vite --port 5173 --force` 清缓存重启后 mock 数据即恢复，与代码无关，特此留档免得后人误判成水合缺陷）：

| 测点 | 实测 |
| --- | --- |
| 置顶槽 x / 工作区图标 x | **6 / 6**（两处都是 15px 槽） |
| 会话名 x / 工作区名 x | **26 / 26**——5 条会话行全是 26，9 个组名全是 26 |
| 每行按钮数 | 5 行全是 **2**（置顶 + 设置） |
| 未置顶的置顶按钮 | `opacity 0 / visibility hidden`（不参与命中测试） |
| 选中行 | `border-top-color: rgba(0,0,0,0)`（透明）+ `box-shadow: inset 3px 0 0 accent, …` |
| 置顶行为 | 点第 4 条「前端重构」→ 组内次序变为它排第一，按钮标签翻为「取消置顶」，`data-pinned="true"`，`pylon-sessions` 里 `pinned: true` 落盘；再点一次全部复原 |

**过程中实测出的 1px 缺陷**：模式级规则 `.session-item:has(.session-actions) { gap:4px }` 把行内间距压到 4px，会话名因此落在 x=25（工作区名 26）。已改回 5px，并在 CSS 契约里加了一条「模式级行间距不得偏离 5px」的断言——那条正是最容易与基线契约悄悄漂开的覆盖。

### 追加轮七（2026-09-19）：窄宽度下「会话设置 / 工作区设置」看不见——折叠动画埋的栅格缺陷

用户提醒「注意下会话设置和工作区设置按钮在左侧栏拖拽压缩宽度下的可见性」，一量就现形，而且**成因正是追加轮五的折叠动画**：

| 事实 | 数值 |
| --- | --- |
| 折叠动画把 `.sidebar-block-body` 从 flex 列改成 `display:grid`，**隐式单列默认按内容定宽** | 左栏 250px 时 `.session-list` 被撑到 **278px**（client 278 / 容器 246） |
| body 的 `overflow:hidden` 于是裁掉右侧 32px | 行尾「会话设置」位于 250–268 → **整段在被裁区**，这就是用户报「完全不见了」的**真正成因**（追加轮六里我只修到「两层压暗」这一半） |
| 修法 | `.sidebar-block-body { grid-template-columns:minmax(0,1fr) }` + `.sidebar-block-body-inner { min-width:0 }` |

**修复后实测**：

| 左栏宽 | `.session-list` | 行右边界 | 会话设置按钮 | 工作区设置按钮 | 会话名 |
| --- | --- | --- | --- | --- | --- |
| **160**（拖拽钳位下限） | client 156 = scroll 156（无横向溢出） | 153（栏内） | 128–146（**栏内**） | 131–153（**栏内**） | 收到 83px 并省略 |
| **250** | client 246 = scroll 246 | 242 | 218–236（**栏内**） | 221–243（**栏内**） | — |

两列对齐不受影响：置顶槽 x=6 == 工作区图标 x=6；会话名 x=26 == 工作区名 x=26。宽度由 store 拖拽到 160 复现、测完已还原为用户的 250。

**测试**：CSS 契约新增「模块体的栅格列必须可收缩（`minmax(0,1fr)`）+ inner `min-width:0`」，并写明这条对应的是「行尾按钮被裁」这个实机缺陷。

### 追加轮八（2026-09-19）：「会话设置不可见」的真正根因（双层门控） + 滚动轨外壁收窄

用户复验仍报「会话设置不可见」。逐层排查后发现**真正根因**：

- 行尾动作的门控是**两层**的：容器 `.session-actions { opacity:0; visibility:hidden }`，加上每个按钮自己的 `opacity:0; visibility:hidden`；
- 而显形规则只提**按钮**的值。`opacity` 作用于**整个子树**——容器 opacity:0 时，按钮自己提到 1 也被整体罩住；
- 旧 CSS 契约甚至把这个错误断言成了正确行为（「按钮自带三层门控」）。修法：门控**只在容器**上（行 hover/focus-within 时容器 opacity:1 + visible），按钮不再自带第二层；触屏媒体查询同步；按钮的 `:hover`/`:focus-visible` 着色保留，另给设置钮真正的 `outline` 焦点环。顺带删掉指向已不存在结构的 `.session-item > .session-action` 直连选择器组。
- 用 CSS 契约把新模型钉住：容器三层门控 + 按钮**不得**再带自己的 opacity/visibility。

**滚动轨外壁收窄**（用户：「我本意是让你收窄外壁宽度」）：聊天右侧的自绘滚动轨从 20px 收到 **12px**（端点按钮 22→16，窄屏 24→14/16），滑块左右内缩 4px→1px（滑块在 12px 轨内约 7–9px 宽，占比反而更饱满）；`--creation-overlay-right-inset` 消费同一 token 自动跟随。实测：rail 12px / track 9px / thumb 7px。

**如实记录的验证缺口**：验收实例（真实后端）没有可用的 Agent 会话，行内按钮的**悬停合成效果**（容器显形 → 按钮可见）只能靠 CSS 契约与计算样式证明（按钮 opacity=1、仅剩容器一层门控），实机目视待用户复验。
