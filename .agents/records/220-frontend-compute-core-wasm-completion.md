...
## 27. A① 收口回执：热归约器走定长部件 —— **实测无可测量收益**

按 2026-09-21 裁决做 A①（让热归约器不再物化事件镜像）。改动本身正确（181 Rust 用例 +
parity 175 项 + 623 文件/4698 用例全绿，fmt/clippy 干净，提交 `b32c5c25`），但**收益是零**：

| 量 | A① 前 | A① 后 | 判读 |
| --- | --- | --- | --- |
| live 逐事件（3 次取样，机器空闲） | 32.0 / 36.5 / 35.6µs | 35.5 / 37.7 / 38.1µs | 同一噪声带，**无改善** |
| ↳ `appendBatch` 单事件 | 14.46–14.86µs | 14.31–15.04µs | 无变化 |
| ↳ 迁移前 TS（对照） | 19.5–20.1µs | 19.54µs | — |
| `bench-memory-hold` ②（核+JS 文档） | 1.59 / 3.04MiB | **1.59 / 3.04MiB** | 逐位相同 |
| `bench-memory-hold` ③（核线性峰值保留） | 8.19 / 14.38MiB | **8.19 / 14.38MiB** | 逐位相同 |

### 为什么没收益（本轮最有用的结论）

`foldPhases` 在**单事件帧**上读数低于毫秒时钟分辨率 ⇒ 一次 live 调用里，**核内
（decode + project + patchJson）只有几 µs**。而 `appendBatch` 单事件 14.3–15.0µs、
`JSON.parse` 2.3µs ⇒ **其余 ≈10µs 是逐次过界的固定开销**（wasm-bindgen 调用、帧字节
拷贝、patch 字符串从 Rust 落地到 JS）。

于是两件事同时成立：
1. 砍掉核内的一次小分配（事件镜像 ≈6 节点 → 1 部件 ≈3 节点），落在 15µs 里**看不见**；
2. `bench-memory-hold` 的 ② 与 ③ 由**峰值 / 分配器高水位**主导，而稳态文档变小
   （组成探针已证 2×）在这两个数上**照不出来** —— ② 的基线本身已含预热那一轮抬起的
   线性内存，量的是基线之后的增量。

### 对两条判决线的更新

- **投影 live（1.8×）**：账在**次数**不在每次的量。唯一杠杆是 A②：把一拍内到达的事件
  合批成一页再折。上限可观（若每帧 8–16 事件，过界次数降同量级），**但它改变可观测的
  更新时序**（文档不再在 ingest 后同步更新），需仓库主点头。
- **投影内存（②/① = 4.4–6.1×）**：A①/A② 都收不回来。它来自「**文档存两份**」
  （Rust 核一份 + JS 物化一份）这一结构事实 —— 要动它只能让 JS 侧不再持有整份文档，
  那是消费侧契约变更。
- **值得保留 A① 的理由**：它不是负收益，是「零可测量收益」—— 逐事件少一次小分配、
  且是「热路径彻底不碰 `Value`」的必经中间态。若最终判定投影回 TS，这层可一并撤。

### 本轮方法学教训（记下来防止重犯）

`bench-live-probe` 的 `append` 段读 `foldPhases()` 只反映**最后一次调用**，而单事件调用
低于毫秒时钟分辨率 ⇒ 该探针在 live 粒度上**没有分辨力**。要拆 live 的核内外，得累计
N 次调用的核内时间（或给 `diag_now_ms` 换更高分辨率时钟），否则只能靠「总时长 − 已知段」
反推。本轮就是靠反推得到「核内只有几 µs」的结论。

---

## 28. 【更正】撤回 A②：投影的「历史 / 现实」之分是既有的语义边界，不是优化余地

仓库主 2026-09-21 指出「投影和重放有历史和现实的区分」。核对代码与说明书后确认**成立**，
并据此**撤回 §24.3 的路线 B（把 live 折叠合批）与我上一轮的相关推荐**。

### 事实（引自仓内）

| 事实 | 出处 |
| --- | --- |
| live 逐帧到达即**单事件页**；冷装载按 journal 页**合批** | `agentWorkbenchSession.ts:405` 的 `foldPage` 头注；live 调用点全是 `foldPage([envelope])`（669/767/828/896/1127），合批只出现在 refresh 页（1055）与回滚重折（707/740） |
| dispatcher 对同 owner 的 live rows 做**有界 32 行 / 8ms 窗口合批**，但位置在 **durable append 之前** | `docs/说明书/Pylon-项目架构参考.md:199` 末句 |
| `canonical_events` 是**重启后的历史权威**；已有 authoritative local rows 时 **local journal wins**，replay 只作诊断/完整性证据，不覆盖、补齐或重排本地事实 | 同上段中段 |
| 信任级别不同：`local-observed/authoritative` **只能由 Kernel live ingest 产生**；Agent replay 仅在可信 journal 为空时作为 `recovery-import/unverified` 写入 | `CONTEXT.md:64` |
| live 的权威**不是文档**：per-runtime turn 终态账本 + `turn_in_flight`（进程内、不落盘）；前端活性优先级 **kernel > clock > document** | 同上段 |

### 更正

1. **A② 不是性能优化，是削弱一条语义边界。** 系统里那个 32 行 / 8ms 的批是**持久化**的批，
   不是**投递**的批——前端收到的仍是逐帧。把 live 折叠也合批，等于让「现实」被「历史」的批
   节奏拖住，而 `kernel > clock > document`、per-frame 投递、durable-before-project 这一整套
   设计正是为了让现实即时可见。**撤回 A②。**
2. **我此前拿错了口径定罪。** 我把 live 逐帧（35µs/事件）与迁移前 TS 逐事件比，得出「投影
   wasm 慢 1.8×」。但逐帧是**设计选择**（现实要即时可见），不是缺陷；投影核的**公平对照是历史
   路径**（页级合批）：`fold mixed-m` **1.08×**、`paged paged-replay-m` **1.84×** —— 接近打平，
   不是「差」。**判决「投影该不该在 wasm」应只由历史路径定**，而历史路径的性能是打平的。
3. **唯一不受这条区分影响的判决线是内存**：文档存两份（Rust 核 + JS 物化）在历史路径上一样付，
   ②/① = 4.4–6.1×。所以判决收敛成一句：**是否愿意为「一份单一实现的计算核」付 5–6× 的文档内存。**

### 一个更锋利的推论（留给出题人，不代决）

若 live 是现实、文档是历史物，那么 live **逐帧重建整份文档**这个形状本身就值得怀疑：live 真正
需要的只是「往最后一条消息追加一段文本」的增量，而渲染层已有揭示调度器。若走这条：

- 投影核退化为**历史引擎**（页级合批、1.08×，可接受）；
- live 的成本与「计算核在哪」**脱钩**；
- 这也解释了 `reduceWorkbenchFold`（单事件纯函数出口）为何存在——它服务纯函数语义与测试，
  未必是 live 的正确形状。
