...
## 性能对照实测（2026-09-21，bun 1.4.0，同一 scratch 脚本跑两棵树：基线 `c88d4fe9` vs `fa037a2c`；脚本为一次性 scratch 不入库，数字不可重跑但测法见下方注记）

**A. live 单事件纯归约器（`reduceWorkbenchEvent`，200 帧 best-of-5，两次采样一致；每帧 µs）**

| 种子文档规模 | 基线 | 改动后 | 加速 |
| --- | --- | --- | --- |
| 5k | 106–110 | 62–74 | ~1.6× |
| 20k | 371–423 | 242–253 | ~1.6× |
| 40k | 767–793 | 563–576 | ~1.4× |
| 80k | 1872–1938 | 1062–1084 | ~1.8× |

注：本段只隔离归约器（扫描项所在层）；含 `applyDocument`（freeze/legacy/分配地板）的
全路径在本口径下无可测端到端差——地板由不可变数组拷贝 + GC 主导，即「未解问题 1」的
结构性残留。增长斜率：基线每翻倍 ~×2.4，改动后 ~×1.9（残留为线性拷贝项，差距随规模扩大）。

**B. 冷重放 bind（20k chunk 单回合 journal，行数 20002 → 聚合 1602）**

| 形态 | 基线 | 改动后 |
| --- | --- | --- |
| 逐 chunk | 921–1625ms | 921–1064ms（控制组） |
| 聚合 batch | 947–1554ms | **277–297ms** |

基线里 batch≈逐 chunk（同一条展开路径）；改动后聚合形态稳定为逐 chunk 的 ~1/3
（跨树对照受运行间噪声影响，取同树批内对照为准）。

**C. bind JS 堆留存（20k chunk，gc 后 heapUsed）**

| 形态 | 基线 | 改动后 |
| --- | --- | --- |
| 逐 chunk | +14.5MiB | +19.2MiB（控制组） |
| 聚合 batch | +23.5MiB | **+4.1MiB**（~1/5） |

foldLog 替换项在本口径下**无可测差**（bind(A)+refresh(B) 留存 54.1 vs 55.9MiB，噪声内）：
引用释放的收益被 Bun 堆计量精度淹没。该项的主要价值是回滚源新鲜度（log=当前 journal
权威集），内存收益按理论存在但不在此口径宣称。

复现注记：与 #205 同例，scratch 已删不入库；重建同形状输入（流式 text.delta 恒定
messageId、每 25 帧一把 tool_call、coverage 单点、sequence 连续）即可复测。
