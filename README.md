...
本文是当前版本的使用与开发说明。实现地图和扩展契约见 [`docs/`](docs/)；施工书、原型等内部过程资料统一归档在本仓库之外的 `Docs` 目录（不随仓库分发），其中已落地的结论应回写到说明书对应章节。
...
从 [Releases](https://github.com/AlchemistCxC/Pylon-co-works/releases/latest) 下载最新的 `pylon-<x.y.z>-win64.zip`，解压后直接运行 `pylon.exe`（便携版免安装）。首次使用需要在 Agent 设置中填写可执行文件、参数和工作目录；配置会保存在应用用户数据目录，不会写入项目仓库。
...
- 问题台账（外部 `Docs/Pylon-问题台账.md`，不随仓库分发）
- 下一阶段问题清单（外部 `Docs/Pylon-下一阶段问题清单.md`，不随仓库分发）
