...
assert.match(source, /shouldAcceptToolCall\(toolId, replayToolIdsRef\.current\[source\]/, 'replay tool_call 必须拒绝缺失和重复 ID')
assert.match(source, /if \(!toolId\) break/, 'tool_call_update 缺失 ID 必须安全忽略')
assert.match(source, /replayToolIdsRef\.current\[source\] =/, 'replay tool ID 必须按 source 隔离记录')
