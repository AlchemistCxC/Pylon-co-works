...
const rightSlot = readFileSync(new URL('../src/workspace-sheets/SheetRightSlot.tsx', import.meta.url), 'utf8')
...
assert.match(rightSlot, /<ContextPanelHost sheet=\{sheet\} ctx=\{ctx\} \/>/, '右栏 slot 必须统一挂载贡献 host')
...
assert.match(app, /const sidebarCollapsed = useWorkspaceStore\(s => s\.sidebarCollapsed\)/, 'App 折叠状态必须直接订阅全局 workspaceStore')
assert.match(layout, /const sidebarCollapsed = useWorkspaceStore\(s => s\.sidebarCollapsed\)/, 'SheetLayout 必须向所有 Sheet 注入同一折叠状态')
assert.match(app, /const sidebarWidth = useWorkspaceStore\(s => s\.sidebarWidth\)/, 'App 宽度必须读 workspaceStore')
assert.match(app, /setSidebarCollapsed\(!sidebarCollapsed\)/, '折叠 toggle 必须写全局布局状态')
const workspaceStore = readFileSync(new URL('../src/workspaceStore.ts', import.meta.url), 'utf8')
assert.match(workspaceStore, /setSidebarCollapsed: \(sidebarCollapsed\) => set\(state => commitWorkspaceMutation\(state, \{ sidebarCollapsed \}\)\)/, '折叠 action 不得再写 active Sheet metadata')
