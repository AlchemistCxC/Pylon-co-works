...
import { getToolSummary } from '../src/components/chat/toolPresentation.ts'
...
assert.equal(resolveToolKind('web_search'), 'search')
...
// ── 状态/摘要/输出保持既有语义（summary 变 title 直通回退） ──
const model = buildToolPresentationModel(toolMessage('Edit', unified), 'completed')
assert.equal(model.summary, 'Edit', '无输入时 summary 回退 title 直通')
