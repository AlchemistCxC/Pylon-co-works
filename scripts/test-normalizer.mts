...
// EVT-04 AC-1：tool 字段经 canonical 单一路径投影（toolFieldsFromCanonical），不再直读 wire 私有键
assert.match(controller, /toolFieldsFromCanonical\(canonical\)/, 'tool 字段必须经 toolFieldsFromCanonical 单一路径提取（AC-1）')
assert.match(controller, /toolKind: tool\.kind/, 'tool_call 必须携带 canonical 投影的 kind（AC-1）')
assert.match(controller, /contentBlocks: tool\.contentBlocks/, 'tool_call 必须携带 canonical 投影的 contentBlocks（AC-1）')
assert.match(controller, /type: 'tool-call-update'[\s\S]*?contentBlocks: tool\.contentBlocks/, 'tool_call_update 必须携带 canonical 投影的 contentBlocks（AC-1）')
