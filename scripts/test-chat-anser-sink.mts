...
assert.match(source, /(?:name === 'Bash'|model\.name === 'Bash') && outputHtml[\s\S]*?dangerouslySetInnerHTML=\{\{ __html: outputHtml \}\}/, 'Bash 输出必须使用已清洗的 outputHtml sink')
assert.match(source, /: <pre><code>\{(?:output|model\.outputText)\}<\/code><\/pre>/, '非 Bash 输出必须继续使用 React 文本节点')
