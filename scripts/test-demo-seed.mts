...
// 7. seed 接线（App effect 顺序守卫）：App.tsx 末尾 effect + 顺序约束
{
  ...
  assert.match(app, /sessions\.length > 0/, '二次启动必须跳过')
  ...
  assert.match(seed, /setAgents\(buildDemoAgents\(\)\)/, 'seed 必须先 setAgents（内部 replaceSheets 清 sheets）')
  ...
  assert.match(seed, /setActiveSession\(sessions\[0\]\.id\)/, 'setActiveSession 必须最后（写回 effect 持久化）')
}
