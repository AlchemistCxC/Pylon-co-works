...
assert(/useSessionLiveStats\(sessionId\)/.test(controlCenter), 'ControlCenter side-effect contract changed: expected session runtime path')
...
assert(/hydrateSessions\(\)/.test(store) && /persist\(/.test(store), 'store side-effect contract changed: expected session hydration/persistence wiring')
