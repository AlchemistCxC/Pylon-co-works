...
assert.match(overview, /openOwnedSessionTransaction\(/, '恢复必须经 owner-aware 打开事务（I01-W4）')
assert.match(overview, /selectSession: id => ctx\.selectSession\(id\)/, '恢复必须 selectSession')
assert.match(overview, /openAgentSheet: \(\{ title, agentId \}\) => ctx\.openSheet\(\{ kind: 'agent'/, '恢复必须 open agent sheet')
assert.match(resumeTxn, /deps\.updateSession\(id, \{/, '创建 row 必须纠正 source/periId（事务内）')
assert.match(overview, /localRecent\.length === 0 && persistedRecent\.length === 0/, '本地与持久化最近会话均为空时显示空态')
