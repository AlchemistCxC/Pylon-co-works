...
assert.match(view, /searchAllMessages\(needle\)/, '必须经统一搜索入口（I14-W4 Tauri 后端 / browser 快照）')
assert.match(view, /searchAllMessages/, '必须经纯域/后端搜索服务')
assert.match(view, /sessionUiStateSet\(result\.sessionId, 'pendingMessageLocation'/, '必须保存持久定位意图（FE-AUD-003）')
assert.match(view, /selectSession: id => ctx\.selectSession\(id\)/, '结果点击必须 selectSession（I01-W4 owner-aware）')
assert.match(view, /openAgentSheet: \(\{ title, agentId \}\) => ctx\.openSheet\(\{ kind: 'agent'/, '结果点击必须 open agent（owner）')
...
assert.match(registry, /kind: 'search'.*sidebarMode: 'sheet'.*component: lazyWorkspace\(SearchSheetView\)/, 'search type 必须注册内置左栏 renderer')
assert.match(view, /!ctx\.sidebarCollapsed && \(/, 'search 内置工具栏必须服从宿主折叠态')
