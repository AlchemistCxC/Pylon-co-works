...
const source = readFileSync('src/renderers/solid-workbench/chat/MessageRow.solid.tsx', 'utf8')
...
requireToken(store, 'const thoughtStartedAt = runtime.thinkingStart', '首个 thought chunk 起始时间记录')
...
// C01：四态 label（redacted → 正在思考 → duration → missing），token 覆盖 Solid signal 分支
requireToken(source, "state() !== 'redacted' && state() !== 'missing'", 'C01 四态分支')
requireToken(source, "return '正在思考…'", '运行中标签')
requireToken(source, 'formatThoughtDuration(props.durationMs)', '完成态时长标签')
requireToken(source, 'data-state={state()}', '运行/完成状态标记（C01 四态）')
requireToken(source, '<span class="term-reasoning-label"', '无前缀 thought 标题')
