...
assert.match(chatView, /resolveReplayEventMode/, 'ChatView 必须使用 replay/live 事件归一化')
assert.match(chatView, /resolveTerminationScope/, 'ChatView 必须区分 replay/live 终止事件')
assert.match(chatView, /settleReplayToolMessages/, '历史 Tool 必须有稳定终态收敛')
assert.match(chatView, /normalizeToolId/, 'Tool 事件必须归一化 ID')
assert.match(chatView, /shouldAcceptToolCall/, 'Tool replay 必须防重复/乱序污染')
assert.match(chatView, /isKnownSource/, '事件必须按已存在 source 路由')
assert.match(chatView, /persistMessageSnapshot/, '后台与当前消息必须走统一持久化入口')
assert.match(chatView, /clearMessageStorage/, 'clear 必须清理消息缓存')
assert.match(chatView, /cancelStateRef/, 'cancel 状态必须按 source 保存')
