...
assert.match(controller, /resolveReplayEventMode/, '事件控制器必须使用 replay/live 事件归一化')
assert.match(controller, /resolveTerminationScope/, '事件控制器必须区分 replay/live 终止事件')
assert.match(controller, /settleReplayToolMessages/, '历史 Tool 必须有稳定终态收敛')
assert.match(controller, /normalizeToolId/, 'Tool 事件必须归一化 ID')
assert.match(controller, /shouldAcceptToolCall/, 'Tool replay 必须防重复/乱序污染')
assert.match(controller, /isKnownSource/, '事件必须按已存在 source 路由')
...
assert.match(controller, /cancelStateRef/, 'cancel 状态必须按 source 保存')
