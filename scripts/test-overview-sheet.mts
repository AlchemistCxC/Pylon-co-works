...
assert.match(layout, /<overviewEntry\.component sheet=\{VIRTUAL_OVERVIEW_SHEET\}/, '空态必须渲染 overview component')

// 5. registry overview 条目渲染 OverviewSheetView
assert.match(registry, /kind: 'overview'.*sidebarMode: 'sheet'.*component: lazyWorkspace\(OverviewSheetView\)/, 'overview type 必须注册内置左栏 renderer')
assert.match(overview, /!ctx\.sidebarCollapsed && \(/, 'overview 内置导航必须服从宿主折叠态')
