...
{
  const current = state('A', 'canceling')
  assert.deepEqual(resolveCancelCommand('A', current), current)
  assert.deepEqual(
    resolveCancelCommand('B', current),
    current,
    'a command result from another source must be ignored',
  )
}
