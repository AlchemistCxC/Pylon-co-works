...
if (!clippyPath || !baselinePath) {
  console.error(
    "usage: node scripts/check-clippy-baseline.mjs <clippy.json> <baseline.json>",
  );
  process.exit(2);
}
...
const removed = [...baseline].filter(
  (fingerprint) => !diagnostics.has(fingerprint),
);
...
if (added.length > 0) {
  console.error(
    `check-clippy-baseline FAILED: ${added.length} new diagnostic(s)`,
  );
  process.exitCode = 1;
}
