...
assert.match(registry, /kind: 'runtime'.*sidebarMode: 'sheet'.*component: lazyWorkspace\(RuntimeSheetView\)/, 'runtime type 必须注册内置左栏 renderer')
assert.match(sheet, /!ctx\.sidebarCollapsed && <aside className="runtime-sidebar">/, 'runtime 内置左栏必须服从宿主折叠态')
