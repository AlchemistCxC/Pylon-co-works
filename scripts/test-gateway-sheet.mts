...
assert.match(gatewayTxn, /scope: 'gateway', config: \{ gateway: \{ routes: routes\.map\(gatewayRouteToConfigEntry\) \} \}/, 'scope=gateway 显式契约必须在保存事务（Phase 3 拍板；I12 W6 写回经 yaml 键适配器）')
