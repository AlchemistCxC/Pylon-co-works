...
const customPresets = readFileSync(new URL('../src/customPresets.ts', import.meta.url), 'utf8')
...
for (const field of ['inputVariant', 'inputShowPlaceholder', 'inputShowHistoryHint', 'inputSubmitButtonMode']) {
  assert.match(presets, new RegExp(`'${field}'`), `${field} 必须进入 cc zone`)
  assert.match(customPresets, new RegExp(`'${field}'`), `${field} 必须进入 custom preset allowlist`)
}
