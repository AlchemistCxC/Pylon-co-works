// ─────────────────────────────────────────────────────────────────────────────
// 【冻结基线】迁移前的 TS 投影折叠引擎（issue #220 计算核 wasm 化）。
//
// 出处：`git show f784dc98:src/domains/workbench/__baselineOldProjector.ts`
//（该文件本身是从 `76cbc819`（三处 TS 引擎退役）的父提交原样取出的）。
// 仅服务 `scripts/compute-parity/` 脚手架的 TS↔wasm 对照，**不在任何生产路径**：
// 没有生产模块 import 它。**不要修改本文件的实现逻辑**——它的价值在于忠实代表
// 迁移前行为；要修投影逻辑请改 Rust 计算核（src-tauri/pylon-compute）。
// 相对 import 已改写为指向 src/domains/workbench/ 的现存模块（仅路径改动）。
// ─────────────────────────────────────────────────────────────────────────────

// ⚠️ 冻结的**性能基线**，不是活实现——issue #220。
//
// 这是迁移前的 TS 投影折叠，从 git 历史（`76cbc819` 的父提交）原样取出，**只**给
// `scripts/bench-ts-vs-wasm.mts` 做「迁移前 vs 迁移后」的同 harness 对照用：
// 用户要求「不得出现用了 wasm 计算核还不如原生 TypeScript 的情况」，而这条判据
// 必须有可复现的对照，不能拿两处不同环境的数字互比。
//
// - **不在任何生产路径上**：没有生产模块 import 它；唯一消费者是上面那个基准脚本。
// - **不要改它**：它的价值就在于「逐字节等于迁移前」。要修投影逻辑请改 Rust 计算核。
// - **用完即删**：性能差距（见 `.agents/records/220-*completion.md`）收口后，
//   本文件与对照脚本一并删除，不留长期双实现（issue 原文纪律）。
//
/**
 ...
 */
import { coalesceAdjacentDisplayTextParts, coalesceAdjacentReasoningParts, createUnknownContentPart, parseContentPart, type ContentPart } from '../../../src/domains/workbench/content/contentPartSchema.ts'
import { applyGoalEvents, applyPlanEvent, createEmptyGoalState, createEmptyPlanState, normalizeGoalSnapshot, type GoalSnapshot, type GoalState, type PlanState } from '../../../src/domains/workbench/plan/goalModel.ts'
import { applyLifecycleEvent, createEmptyLifecycleState, normalizeNormalizedError, type LifecycleState, type NormalizedError } from '../../../src/domains/workbench/lifecycle/lifecycleModel.ts'
import { redactInteractionEvent, reduceInteraction } from '../../../src/domains/workbench/interactionProjection.ts'
import { isActivityStatus } from '../../../src/domains/workbench/events/workbenchEventSchema.ts'
import type {
  ActivityEvent,
  AssistEvent,
  DiagnosticEvent,
...
import {
  ...
  type UsageSnapshot,
} from '../../../src/domains/workbench/session/sessionSurface.ts'
