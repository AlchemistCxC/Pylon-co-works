// workbench 语义 envelope 生成器（projector 域的输入面）。
//
// 出处：`fa4aab5d^:scripts/compute-parity/fixtures/envelopes.ts`，随本 issue 从 git 历史
// 恢复——projector 的对照套件在 ADR-0018 修订 1（投影回退 TS）时删除，但**输入面**与
// 「投影已接线到哪」无关，是纯 fixture，故按「已接线的存量测试可借」收编。
// 相对导入深度与出处相同（`scripts/*/fixtures/` → 仓根 → `src/`）。
//
// envelope 构造口径照抄 `src/domains/workbench/__tests__/projectorComputeParity.test.ts`
// 的 `envelope()` helper（用生产 API `createWorkbenchEnvelope`）；事件载荷形状同该测试的
// 混合流语料（message/reasoning/tool/interaction/diagnostic/session/event.unknown 全族覆盖）。
...
export const SESSION_ID = 'perf-bench'
