...
assert.match(lifecycle, /if \(!disposed\) useStore\.getState\(\)\.setAgents\(/, 'list_agents 结果必须受 disposed guard 保护')
...
assert.match(store, /setAgentStatus: \(id: string, status: import\('\.\/components\/settings\/agentTypes'\)\.AgentStatus\)/)
assert.match(store, /setAgentStatus: \(id, status\) => set\(state => \(\{ agentStatuses: \{ \.\.\.state\.agentStatuses, \[id\]: status \} \}\)\)/)
