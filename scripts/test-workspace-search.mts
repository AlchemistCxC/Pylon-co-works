...
assert.match(panel, /provider\.search\(target, query\.trim\(\)\)/, '必须只经 FileProvider 调正式 workspace_search 命令')
...
assert.match(view, /listFileActivities\(target\)/, '搜索分区必须由 activity registry 接线')
assert.match(view, /onOpenFile=\{openFileTab\}/, 'activity 结果点击必须打开 tab')
assert.doesNotMatch(view, /import WorkspaceSearchPanel/, 'FileSheet 核心不得 import 固定搜索实现')
