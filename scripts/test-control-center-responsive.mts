...
assert.equal(controlCenter.includes('<span className="cc-command-hint-key">/: 命令</span> <i>|</i> Shift+Enter: 换行'), true)
assert.equal(controlCenter.includes("cliHintMode === 'full' && <><i>|</i> Shift+Tab: 模式</>"), true)
...
assert.equal(css.includes('padding:0 calc(12px + var(--cc-right-inset, 0px)) 3px 12px'), true)
assert.equal(css.includes('row-gap:0'), true)
assert.equal(css.includes('min-height:32px'), true)
assert.equal(css.includes('var(--cc-right-inset, 0px)'), true)
assert.equal(controlCenter.includes("'--cc-right-inset': rightOpen ? `${rightWidth}px` : '0px'"), true)
