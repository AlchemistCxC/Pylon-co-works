...
assert.match(view, /client\.exportSession\(\{ agentId: owner\.agentId, periId, format: 'markdown', outputPath \}\)/, '导出必须带绝对路径参数')
...
assert.match(registry, /kind: 'history'.*sidebarMode: 'sheet'.*component: lazyWorkspace\(HistorySheetView\)/, 'history type 必须注册内置左栏 renderer')
assert.match(view, /!ctx\.sidebarCollapsed && \(/, 'history 内置分页导航必须服从宿主折叠态')
