...
// Structural render contracts: each editable control is identified and
// associated with its label; select uses the shared accessible component and
// unknown values use JSON.stringify in a code node.
...
assert.match(fieldSource, /disabled=\{disabled\} onChange=\{onChange\}/)
assert.match(fieldSource, /option\.options\.map\(choice => \(\{ value: choice\.id, label: choice\.label \}\)\)/)
