...
import { graphemeWidth, stringWidth, truncateStartToWidth, truncateToWidth } from '../src/utils/textWidth.ts'
import { recursivelyStripHiddenUnicode, stripHiddenUnicode } from '../src/utils/unicodeSanitizer.ts'
import { blinkVisible } from '../src/components/chat/useBlink.ts'
...
assert.equal(truncateStartToWidth('abcdef', 4), '…def', '保留尾部截断')
assert.equal(truncateStartToWidth('中文字符', 4), '…符', 'CJK 尾部截断按宽度')
...
assert.deepEqual(recursivelyStripHiddenUnicode(['a\u200b', { k: 'b\u202e' }, 42]), ['a', { k: 'b' }, 42], '递归对象/数组')

// ── D5 blink 纯逻辑 ──
assert.equal(blinkVisible(0, 600), true)
assert.equal(blinkVisible(600, 600), false)
assert.equal(blinkVisible(1200, 600), true, '半周期切换')
assert.equal(blinkVisible(300, 600), true)
...
assert.match(footer, /const activity = idleMs > 10000 \? 'stalled'/, 'stalled 阈值保持')

// ── D4 Pet polling 接线断言（normal 不 setState） ──
const pet = readFileSync(new URL('../src/components/PetCompanion.tsx', import.meta.url), 'utf8')
assert.match(pet, /setPet\(previous => \{[\s\S]*?JSON\.stringify\(persistable\(previous\)\) === serialized\) return previous/, 'Pet 轮询数据无变化必须返回旧引用')
