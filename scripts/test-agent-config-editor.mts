...
// 1. command missing → blocked（保存命令不可用）
...
assert.match(editor, /保存命令不可用/, '命令缺失必须明确展示阻塞态')
assert.match(editor, /listAgents\(\)/, '保存成功后必须刷新 Agent 列表')
...
console.log('agent config editor 守卫通过')
