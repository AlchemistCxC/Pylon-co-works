...
assert.equal(app.includes("'--right-panel-inset': `${rightPanelInset}px`"), true)
assert.equal(app.includes('<ChatView sessionId={activeSession} />'), true)
...
assert.equal(app.includes('<PetCompanion rightInset={rightPanelInset} />'), true)
