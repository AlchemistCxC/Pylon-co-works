...
assert.equal(agentSheet.includes("'--right-panel-inset': '0px'"), true)
assert.equal(agentSheet.includes('sessionId={ctx.activeSession}'), true)
assert.equal(agentSheet.includes('workspaceMode={workspaceMode}'), true)
...
assert.equal(agentSheet.includes('<PetCompanion rightInset={0} />'), true)
