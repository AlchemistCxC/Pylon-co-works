...
const DEFAULT_CODEG_SRC = 'F:/Hermes/profiles/riccati/workspace/pylon-survey-2026-09/codeg-src'
...
if (manifest) {
  for (const key of ['licenseFile', 'noticeFile']) {
    ...
    if (!existsSync(resolve(root, declared))) fail(`provenance.source.${key} 指向的文件不存在：${declared}`)
  }
  if (manifest.source?.commit !== LOCKED_COMMIT) {
    fail(`provenance.source.commit=${manifest.source?.commit} 与锁定 commit ${LOCKED_COMMIT} 不一致`)
  }
  if (manifest.source?.license !== 'Apache-2.0') {
    fail(`provenance.source.license 应为 Apache-2.0，实为 ${manifest.source?.license}`)
  }
}
...
if (manifest && Array.isArray(manifest.files)) {
  for (const entry of manifest.files) {
    ...
    if (sha256(absolute) !== entry.sha256) {
      fail(`sha256 不匹配：${vendored}（登记 ${entry.sha256}，实际 ${sha256(absolute)}）`)
    }
    if (typeof entry.sourcePath !== 'string' || entry.sourcePath.length === 0) {
      fail(`${vendored} 缺少 sourcePath`)
    }
    if (typeof entry.modifications !== 'string' || entry.modifications.length === 0) {
...
if (existsSync(pylonSrc)) {
  for (const file of walk(pylonSrc, [])) {
    ...
    if (/\bcodeg\w*/i.test(text)) {
      fail(`Pylon 源码出现 codeg 产品命名：${relative(root, file).replaceAll('\\', '/')}`)
    }
  }
}
...
if (manifest && existsSync(codegSrc)) {
  try {
    const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: codegSrc, encoding: 'utf8' }).trim()
    if (head !== LOCKED_COMMIT) fail(`调研副本 HEAD=${head} 与锁定 commit 不一致`)
    for (const entry of manifest.files ?? []) {
  ...
  notes.push(`未找到调研副本 ${codegSrc}，跳过上游 commit/sha256 校验（仓库内事实仍已校验）`)
}
