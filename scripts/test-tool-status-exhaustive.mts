...
// 别名分支（防回退）
assert.equal(normalizeToolStatus('canceled'), 'cancelled', "单 l 'canceled' → cancelled")
assert.equal(normalizeToolStatus('success'), 'completed', "'success' → completed")
assert.equal(normalizeToolStatus('pending'), 'queued', "'pending' → queued")
assert.equal(normalizeToolStatus('error'), 'failed', "'error' → failed")
assert.equal(normalizeToolStatus('in_progress'), 'running', "'in_progress' → running")
