...
assert.match(inputBar, /const generating = useStore\(s => sessionSource != null && \(s\.liveGeneratingSources \|\| \[\]\)\.includes\(sessionSource\)\)/, '生成态必须按当前 Session.source 读取')
...
assert.match(inputBar, /invoke\('cancel_prompt', \{ source: sessionSource \}\)/, '取消 command 必须使用解析后的 Session.source')
