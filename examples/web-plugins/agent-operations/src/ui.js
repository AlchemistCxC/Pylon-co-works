...
export function mountOperations(container,store,adapter,{asset,compact=false}={}) {
  ...
  function render(){
    ...
    for(const a of s.agents){
      ...
      const r=ROOMS.find(r=>r.id===a.room)||ROOMS[3],peers=s.agents.filter(x=>x.room===a.room),slot=peers.indexOf(a);
      const x=r.x-9+(slot%3)*10,y=r.y+7+Math.floor(slot/3)*6;
